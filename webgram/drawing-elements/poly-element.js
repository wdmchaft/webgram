...
Webgram.DrawingElements.PolyElement.prototype = {
    ...
    pointInside: function (point) {
        if (this.shape.points.length <= 1) { /* a single point or nothing - either way the answer is no */
            return false;
        }
        
        if (this.closed) {
            return this.rotateDirect(this.shape).pointInside(point);
        }
        else {
    ...
    getLineTolerancePoly: function () {
        ...
        if (this.shape.points.length === 0) {
            ...
            var point = this.translateInverse(this.shape.points[0]);
        ...
        else if (this.shape.points.length === 2) {
            /* just two points */
            var p1 = this.translateInverse(this.shape.points[0]);
            var p2 = this.translateInverse(this.shape.points[1]);
     ...
     * @param {Webgram.Geometry.Point} point the point to add, relative to the parent
     */
    addPoint: function (index, point) {
        if (index < 0 || index > this.shape.points.length) {
            return;
        }
        ...
        this.shape.points.splice(index, 0, point);
    ...
    remPoint: function (index) {
        ...
        if (this.shape.points.length <= 2) { /* we need at least two points in a polyElement */
            return;
        }
        ...
        var point = this.shape.points[index];
    ...
    setPoint: function (index, point) {
        this.setRotatedShapePoint(index, point);
    ...
    continueCreate: function (point, size, mouseDown, click) {
        ...
        var controlPoint = this.getPolyControlPointByIndex(this.shape.points.length - 1);
        if (controlPoint != null) { /* a control point has been created for the last point */
            ...
            this.shape.points[this.shape.points.length - 1] = point;
        }
        ...
        if (click) {
            ...
            if (this.shape.points.length >= this.maxPointCount) {
                this._pendingCreatePoint = false;
                return false; /* stop */
            }
            
            this.addPoint(this.shape.points.length, point);
            
            controlPoint = this.getPolyControlPointByIndex(this.shape.points.length - 1);
            if (controlPoint != null) {
                controlPoint.move(point, true);
            }
        }
    ...
    endCreate: function () {
        if (this._pendingCreatePoint) {
            ...
            if (controlPoint != null) { /* we have a control point for this point */
                ...
                this.remPoint(this.shape.points.length - 1);
            }
        }
        
        if (this.shape.points.length < this.minPointCount) {
            return false; /* failed */
        }
    ...
    shapeFromJson: function (json) {
        ...
        /* call addPoint(), remPoint() and setPoint() according to the shape changes */
        if (points.length > this.shape.points.length) { /* points were added */
            for (i = 0; i < this.shape.points.length; i++) {
                this.setPoint(i, points[i]);
            }
            
            for (i = this.shape.points.length; i < points.length; i++) {
                this.addPoint(this.shape.points.length, points[i]);
            }
        }
            ...
            while (this.shape.points.length > points.length) {
                this.remPoint(this.shape.points.length - 1);
            }
        }
    ...
    getNextPolyControlPoint: function (polyControlPoint) {
        if (polyControlPoint.polyPointIndex === this.shape.points.length - 1) {
    ...
    getPrevPolyControlPoint: function (polyControlPoint) {
        if (polyControlPoint.polyPointIndex === 0) {
            return this.getPolyControlPointByIndex(this.shape.points.length - 1);
        }
        else {
            return this.getPolyControlPointByIndex(polyControlPoint.polyPointIndex - 1);
        }
    ...
    _snapToAngleInternally: function (index, point, rotationAngle, center) {
        ...
        var bestSolution = null, snappedPoint;
        for (i = 0; i < solutions.length; i++) {
            ...
            if (solution.m != null) { /* the straight line between prevPoint and nextPoint */
                ...
                var snappedPoint = new Webgram.Geometry.Point(solution.x, solution.y);
    ...
    _getHoveredPointIndex: function () {
        ...
        var points = this.rotateDirect(this.shape).points;
    ...
    _computeHoveredPoint: function (mousePoint) {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            if (Webgram.Utils.pointInSegment(mousePoint, point1, point2, this.getLineTolerance())) {
                ...
                return this.transformInverse(hoveredPoint);
            }
        }
...
Webgram.DrawingElements.PolyElement.PolyControlPoint.prototype = {
    ...
    computeAnchor: function () {
        return this.drawingElement.rotateDirect(this.drawingElement.shape.points[this.polyPointIndex]);
    },
    ...
    snap: function (point) {
        if (this._markedForRemoval != null) {
            ...
            var snapDistance = this.getSetting('snapDistance', this.radius);
    ...
    processMove: function (point, vanillaPoint) {
        this.drawingElement._setRotatedShapePoint(this.polyPointIndex, point);
    ...
    _checkAndMarkForRemoval: function () {
        if (this.drawingElement.isCreating()) {
            return;
        }
