...
/**
 * @class The abstract base class for all control points.
 ...
 */
Webgram.ControlPoint = function () {
    /**
     * A control point has a radius, used to determine if the mouse is inside
     * inside it. This radius does not change with the zoom level.
     * @type Number
     */
    this.radius = 9;
    ...
    this._snapToGridEnabled = false;


    /* events */

    /**
     * An event that is triggered when the control point moving begins.<br>
     ...
     * @type Webgram.Event
     */
    this.onUpdate = new Webgram.Event('update', this); /* (deltaX, deltaY) */
     ...
     * @type Webgram.Event
     */
    this.onMouseEnter = new Webgram.Event('mouse enter', this); /* () */
     ...
     * @type Webgram.Event
     */
    this.onMouseLeave = new Webgram.Event('mouse leave', this); /* () */
    ...
    /**
     * An event that is triggered when the control point is selected by the active {@link Webgram.DrawingControl}.<br>
     * Handlers receive no arguments. 
     * @type Webgram.Event
     */
    ...
    /**
     * An event that is triggered when the control point is unselected by the active {@link Webgram.DrawingControl}.<br>
     * Handlers receive no arguments. 
     * @type Webgram.Event
     */
...
Webgram.ControlPoint.FOCUS_NONE = 0;
/** Represents the hovered focus type of a control point. */
Webgram.ControlPoint.FOCUS_HOVERED = 1;
/** Represents the selected focus type of a control point. */
Webgram.ControlPoint.FOCUS_SELECTED = 2;

Webgram.ControlPoint.prototype = {
    /**
    ...
    drawArc: function (center, radiusX, radiusY, startAngle, endAngle) {
        ...
        this.drawingElement._parent.drawArc(center, radiusX, radiusY, startAngle, endAngle);
    },
    ...
    /**
     * Returns the <em>bounding rectangle</em> of this control point.
     * The bounding rectangle is defined as the rectangle circumscribed
     * to the circle having the radius equal to the <tt>radius</tt> property
     * of the control point and centered in the actual center of the control point.<br><br>
    ...
    getBoundingRectangle: function () {
        ...
        /*
         * this is just an adjustment to compensate the offset of the CP's rectangle
         * when zooming in
         */
        var driftOffset = this.getZoomFactor() / 50;
        
        var anchor = this.getAnchor();
        
        return new Webgram.Geometry.Rectangle(anchor.x - radius + driftOffset, anchor.y - radius + driftOffset, anchor.x + radius + driftOffset, anchor.y + radius + driftOffset);
    },
     ...
     * return the anchor point, relative to the parent of this control point's element.<br><br>
     ...
     * corresponding fields on the drawing element based on <tt>point</tt> and <tt>vanillaPoint</tt> arguments.<br><br>
     * <em>(must be overridden)</em>
     * @param {Webgram.Geometry.Point} point the point where the the control point was dragged with the mouse;
     *  this argument contains the value after snapping and applying other limitations; its value is relative to the
     *  element's parent
     * @param {Webgram.Geometry.Point} vanillaPoint the exact point of the mouse where the control point was dragged;
     *  this argument holds the value before any snapping or alteration; its value is relative to the element's parent
     */
    ...
    move: function (point) {
        ...
        point = point.getRotated(this.drawingElement.getRotationAngle(), center);
        
        point = this.snap(point);
        this.processMove(point, center);
    ...
    update: function () {
        ...
        if (oldAnchor && (this._anchor.x !== oldAnchor.x || this._anchor.y !== oldAnchor.y)) {
            this.onUpdate.trigger(this._anchor.x - oldAnchor.x, this._anchor.y - oldAnchor.y);
        }
    },
    
    /**
     * Snaps the control point. The default implementation
     * simply snaps to <em>grid</em>, if enabled.
     * @param {Webgram.Geometry.Point} point the point to snap
     * @returns {Webgram.Geometry.Point} the snapped point
     */
    snap: function (point) {
        if (this.isSnapToGridEnabled() && this.getSetting('snapGrid') != null) {
    ...
    snapToGrid: function (point) {
        ...
        point = new Webgram.Geometry.Point(Math.round(point.x / snapGrid.sizeX) * snapGrid.sizeX, Math.round(point.y / snapGrid.sizeY) * snapGrid.sizeY);
    ...
    /**
     * Tells whether snapping to grid is enabled for this control point or not.
    ...
    /**
     * Enables or disables snapping to grid for this control point.
     * When snapping to grid is enabled, changes to the anchor will
     * be followed by a call to {@link Webgram.ControlPoint#snapToGrid}.
...
Webgram.Class('Webgram.ControlPoint');
