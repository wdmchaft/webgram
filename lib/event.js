...
Webgram.Event = Webgram.Class.extend(/** @lends Webgram.Event.prototype */ {
    /**
     * Manages the bindings between a library <em>event</em>
     * and one or more <em>handler</em> functions. Events can be of any origin and kind,
     * not only those that come from the mouse or the keyboard.
     * @constructs Webgram.Event
     * @extends Webgram.Class
     * @param {String} name a name given to the event (helps when debugging)
     * @param {Object} object the object that owns and triggers the event
     */
    initialize: function Event(name, object) {
        /**
    ...
    unbind: function (handler) {
        if (handler) {
            ...
            Webgram.Utils.forEach(this.handlers, function (h) {
                if (h.func !== handler) {
                    dontRemove.push(h);
                }
            });
    ...
    trigger: function () {
        ...
        for (var i = 0; i <  this.handlers.length; i++) {
            ...
            if (result === false) {
                /* returning false from an event handler will stop the event from being treated further */
                break;
            }
        }
