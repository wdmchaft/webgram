...
Webgram.Geometry = {
    ...
    normalizeAngle: function (angle) {
        // TODO this should not transform the angle into degrees!
...
Webgram.Geometry.Line = Webgram.Class.extend( /** @lends Webgram.Geometry.Line.prototype */ {
    ...
    /**
     * Returns the point on the line having the given <tt>x</tt> coordinate.
     * If no such point exists, returns <tt>null</tt>.
     * @param {Number} x the x coordinate of the point
     * @returns {Webgram.Geometry.Point} the point on the line with the given x coordinate
     */
    ...
    /**
     * Returns the point on the line having the given <tt>y</tt> coordinate.
     * If no such point exists, returns <tt>null</tt>.
     * @param {Number} y the y coordinate of the point
     * @returns {Webgram.Geometry.Point} the point on the line with the given y coordinate
     */
    ...
    intersectLine: function (line) {
        if (this.slope === line.slope) {
            /* two lines of same slope don't intersect in a single point */
        ...
        else { /* this is oblique */
            if (line.isVertical()) {
                /* oblique and vertical */
                
                return new Webgram.Geometry.Point(line.intercept, this.getY(line.intercept));
            }
            else if (line.isHorizontal()) {
                /* oblique and horizontal */
                
                return new Webgram.Geometry.Point(this.getX(line.intercept), line.intercept);
            }
            else {
...
/** @lends Webgram.Geometry.Line */ {
    /**
     * Constructs a line formed by two given points.
    ...
    /**
     * Constructs a line with a given slope that passes through a given point.
     * @param {Webgram.Geometry.Point} point the point on the line
     * @param {Number} slope the slope of the line
     * @returns {Webgram.Geometry.Line} the line formed by the point and the slope 
     */
    fromPointAndSlope: function (point, slope) {
        var intercept;
        if (isFinite(slope)) {
...
Webgram.Geometry.Rectangle = Webgram.Class.extend( /** @lends Webgram.Geometry.Rectangle.prototype */ {
     ...
     * @param {Number} sx the x scaling factor
     * @param {Number} sy the y scaling factor
     * @returns {Webgram.Geometry.Rectangle} the scaled rectangle
     */
    getScaled: function (sx, sy) {
        return new Webgram.Geometry.Rectangle(
                this.x1 * sx, this.y1 * sy,
                this.x2 * sx, this.y2 * sy);
    },
...
Webgram.Geometry.Poly = Webgram.Class.extend( /** @lends Webgram.Geometry.Poly.prototype */ {
    ...
    getCenter: function () {
        ...
        Webgram.Utils.forEach(this.points, function (point) {
            cx += point.x;
            cy += point.y;
        });
    ...
    getTranslated: function (dx, dy) {
        ...
        Webgram.Utils.forEach(this.points, function (point) {
            newPoints.push(point.getTranslated(dx, dy));
        });
    ...
    getRotated: function (angle, center) {
        ...
        Webgram.Utils.forEach(this.points, function (point) {
            newPoints.push(point.getRotated(angle, center));
        });
    ...
    getScaled: function (sx, sy) {
        ...
        Webgram.Utils.forEach(this.points, function (point) {
            newPoints.push(point.getScaled(sx, sy));
        });
