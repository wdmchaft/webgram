...
/**
 * @class A class representing a line in the two-dimensional space.
 ...
 */
Webgram.Geometry.Line = function (slope, intercept) {
    /**
...
Webgram.Geometry.Line.prototype = {
    /**
    ...
    getRotated: function (angle, center) {
        ...
        var slope, intercept, dist, newAngle;
        if (isFinite(this.slope)) {
            /* compute the new slope */
            var tan = Math.tan(angle);
            slope = (line.slope + tan) / (1 - line.slope * tan);
            if (Math.abs(slope) > 10e6) {
                slope = Infinity;
            }
            
            /* compute the new intercept */
            dist = line.intercept * Math.cos(Math.atan(line.slope));
            newAngle = Math.atan(line.slope) + angle;
            intercept = dist / Math.cos(newAngle);
        }
        else { /* vertical line */
            /* compute the new slope */
        ...
        if (isFinite(slope)) {
            ...
            line = new Webgram.Geometry.Line(slope, dist);
        }
