...
if (CanvasRenderingContext2D.prototype.setLineDash == null) {
...
Webgram.Canvas.prototype = {
    ...
    drawText: function (text, box, textStyle, transformSet) {
        ...
        for (var i = 0; i < transformSet._transforms.length; i++) {
            var transform = transformSet._transforms[i];
            if (transform[0] == 's') {
                if (transform[1] < 0) {
        ...
        /* compensate the negative scaling (flipping) */
        if (flippedHorizontally) {
            this.context.translate(box.x1 + box.x2, 0);
            this.context.scale(-1, 1);
        }
        if (flippedVertically) {
            this.context.translate(0, box.y1 + box.y2);
            this.context.scale(1, -1);
        }
    ...
    _actualDrawArc: function (center, radiusX, radiusY, startAngle, endAngle, bounds, transformSet) {
        ...
        if (this.context._strokePattern && this.context._strokePattern.length &&
            ...
            if (radiusX != radiusY) { /* elliptical arc */
                ...
                this.context.arc(center.x, center.y, radiusX, startAngle, endAngle, false);
