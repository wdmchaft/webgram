...
Webgram.DrawingElements.RootContainer =
    ...
    initialize: function RootContainer(webgram, canvas) {
        this.callSuper(':special:root-container');
    ...
    drawSnapVisualFeedback: function () {
        ...
        if (!this.getSetting('snapVisualFeedbackTimeout')) {
            return;
        }
    ...
    _addDrawingElement: function (drawingElement, where, triggerEvents) {
        this.callSuper(drawingElement, where, triggerEvents);
    ...
    _remDrawingElement: function (drawingElement, triggerEvents) {
        ...
        this.callSuper(drawingElement, triggerEvents);
    },
    ...
    setSnapVisualFeedback: function (snapVisualFeedback) {
        ...
        this._snapVisualFeedbackTimer = setTimeout(function () {
            that._snapVisualFeedback = null;
            that.invalidate();
        }, this.getSetting('snapVisualFeedbackTimeout'));
    }
});
...
Webgram.DrawingElements.RootContainer.AngularSnapVisualFeedback =
     ...
     */
    initialize: function AngularSnapVisualFeedback(origin, angle) {
        this._origin = origin;
        this._angle = Webgram.Geometry.normalizeAngle(angle);
    },
    ...
    draw: function (rootContainer, visibleArea, strokeStyle, fillStyle, textStyle) {
        ...
        var arrowTop = this._origin.getPointAt(radius / zoomFactor, this._angle);
        var arrowLen = 10 / zoomFactor;
        var arrowAngle = Math.PI / 4;
        var arrowAngleOffset = 5 / radius;
        
        /* draw the arc */
        rootContainer.drawArc(this._origin, radius, radius, this._angle - angleDelta, this._angle + angleDelta, 0);
        rootContainer.paint(strokeStyle, null);

        /* draw the first arrow */
        rootContainer.drawLine(arrowTop, arrowTop.getPointAt(arrowLen, this._angle - Math.PI / 2 - arrowAngle / 2 - arrowAngleOffset));
        rootContainer.drawLine(arrowTop.getPointAt(arrowLen, this._angle - Math.PI / 2 + arrowAngle / 2 - arrowAngleOffset));
        rootContainer.drawLine(arrowTop);
        ...
        /* draw the angle axis */
        rootContainer.drawLine(this._origin, this._origin.getPointAt(radius * 100, this._angle));
        rootContainer.paint(strokeStyle, null);
    }
});
