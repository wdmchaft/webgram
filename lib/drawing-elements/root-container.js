...
//                this.drawText(textAngle + String.fromCharCode(0xB0), textBox, textStyle);
    ...
    drawRulers: function () {
        ...
        if (!this._mini) {
            ...
            if (showZoom) {
                ...
                /* draw the zoom text */
                var text;
                if (this._zoomFactor >= 2) {
                    text = '' + this._zoomFactor + ' x';
                }
                else {
                    text = '' + this._zoomFactor.toFixed(1) + ' x';
                }
    ...
    draw: function () {
        ...
        for (i = 0; i < this.drawingElements.length; i++) {
            ...
            /* avoid drawing elements that have no shape points */
            if (drawingElement.shape.points.length >= drawingElement.minPointCount) {
                drawingElement.draw();
    ...
    drawArc: function (center, radiusX, radiusY, startAngle, endAngle) {
        if (this._noZoom) {
            ...
            radiusX *= this._zoomFactor;
            radiusY *= this._zoomFactor;
        }
    ...
    setZoomLevel: function (zoomLevel, point) {
        ...
        /* update all the control points - the offsets might have changed with the zoom */
        for (var i = 0; i < this.drawingElements.length; i++) {
            this.drawingElements[i].invalidatePoints();
        }
