...
Webgram.DrawingElements.RootContainer =
    ...
    initialize: function RootContainer(webgram, canvas) {
        Webgram.DrawingElements.RootContainer.parentClass.call(this);
        
        this.setId('@root-container');
        ...
        /* zooming & panning */
        this._noZoom = false; /* true while the zoom-independent drawing happens */
    ...
    getBoundingRectangle: function (transformed) {
        if (this._drawingElements.length > 0) {
            return Webgram.DrawingElements.RootContainer.parent.getBoundingRectangle.call(this, transformed);
        }
        else {
    ...
    _remDrawingElement: function (drawingElement, triggerEvents) {
        if (triggerEvents) {
            /* if the removed drawing element is among the
             * selected elements, an onSelectionChange event has to be triggerd */
    ...
    setZoomLevel: function (zoomLevel, point) {
        ...
        /* update all the drawing elements - the offsets of the various points might have changed with the zoom */
    ...
    /**
     * Tells whether the zoom-independent drawing is being processed or not.
     * @returns {Boolean} <tt>true</tt> if the zoom-independent drawing is being processed,
    ...
    /**
     * Tells whether the drawing is being done on the mini Webgram or not.
     * @returns {Boolean} <tt>true</tt> if the drawing is being done on the mini Webgram,
     * <tt>false otherwise</tt>.
     */
    isMini: function () {
        return this._mini;
...
Webgram.DrawingElements.RootContainer.CompositeSnapVisualFeedback =
    ...
    draw: function (rootContainer, visibleArea, strokeStyle, fillStyle, textStyle) {
        ...
        Webgram.Utils.forEach(this._snapVisualFeedbacks, function (snapVisualFeedback) {
            snapVisualFeedback.draw(rootContainer, visibleArea, strokeStyle, fillStyle, textStyle);
        });
    }
});
