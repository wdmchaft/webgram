...
Webgram.DrawingElements.RectangularElement = Webgram.DrawingElement.extend( /** @lends Webgram.DrawingElements.RectangularElement */ {
    ...
    initialize: function RectangularElement(id, width, height) {
        ...
        Webgram.DrawingElements.RectangularElement.parentClass.call(this, id);
    ...
    getBoundingRectangle: function () {
        ...
        return new Webgram.Geometry.Rectangle(-width2, -height2, width2, height2);
    },
    ...
    fit: function (rectangle, test) {
        ...
        if (!test) {
            this._setLocation(center, false, false);
        }
        ...
        if (parseInt(k) === k) {
            ...
            if (rectWidth > rectHeight && alphaReduced > delta) {
                rectWidth = rectHeight / Math.tan(alphaReduced);
            }
            if (rectWidth < rectHeight && alphaReduced < delta) {
                rectHeight = rectWidth * Math.tan(alphaReduced);
            }
        ...
        if (!test) {
            ...
            /* the size might have changed due to constraints */
            width = this.getWidth();
            height = this.getHeight();
        }
        else {
            /* apply the constraints to the computed dimensions */
