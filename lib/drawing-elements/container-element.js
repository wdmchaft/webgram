...
Webgram.DrawingElements.ContainerElement.prototype = {
    ...
    finishShapeChange: function (force) {
        Webgram.DrawingElement.prototype.finishShapeChange.call(this, force);
        
        for (var i = 0; i < this.drawingElements.length; i++) {
            this.drawingElements[i].finishShapeChange(force);
        }
    },
    
    setRotatedShapePoint: function (index, point) {
        Webgram.DrawingElements.RectangularElement.prototype.setRotatedShapePoint.call(this, index, point);
    ...
    updatePoints: function () {
        Webgram.DrawingElement.prototype.updatePoints.call(this);
        
        for (var i = 0; i < this.drawingElements.length; i++) {
            var drawingElement = this.drawingElements[i];
            if (drawingElement._updatePoints) {
                drawingElement.updatePoints();
                drawingElement._updatePoints = false;
            }
        }
    },
