...
Webgram.DrawingElements = {
    /* a registry for all drawing element classes */
    
    _classes: {},
    
    registerClass: function (cls, name) {
        /* registers the given drawing element class
         * and returns the assigned name of the class
...
Webgram.DrawingElements.ContainerElement = 
    ...
    initialize: function ContainerElement() {
        ...
        Webgram.DrawingElements.ContainerElement.parentClass.call(this);
    },
     ...
     * See {@link Webgram.DrawingElements.ContainerElement#drawSelf} for
     * an explanation.
     */
    ...
    getBoundingRectangle: function (transformed) {
        ...
        var rectangle = new Webgram.Geometry.Rectangle(minX, minY, maxX, maxY);
        if (transformed) {
    ...
    _addDrawingElement: function (drawingElement, where, triggerEvents) {
        var oldParent = drawingElement._parent;
        var absTransforms = null;
        
        if (this.webgram) {
            this.webgram.beginUndoGroup(); /* group the undo all the following undo operations */
        }
        
        if (oldParent) {
            absTransforms = drawingElement.getAbsTransforms();
            oldParent._remDrawingElement(drawingElement, triggerEvents);
        }
        ...
        if (this.webgram && !drawingElement.isBeingCreated() && this.webgram.userInteraction()) {
        ...
        if (oldParent) {
            /* restore the apparent (absolute) geometry */
            
            var thisAbsTransforms = this.getAbsTransforms();
            
            // TODO this will trigger geometry change events on elements that don't have various geometrical operations enabled
            if (absTransforms.flippedHorizontally !== Boolean(drawingElement.isFlippedHorizontally() ^ thisAbsTransforms.flippedHorizontally)) {
                drawingElement._flipHorizontally();
            }
            if (absTransforms.flippedVertically !== Boolean(drawingElement.isFlippedVertically() ^ thisAbsTransforms.flippedVertically)) {
                drawingElement._flipVertically();
            }
            
            var rotationAngle = absTransforms.rotationAngle - thisAbsTransforms.rotationAngle;
            drawingElement._setRotationAngle(rotationAngle);
            
            var location = new Webgram.Geometry.Point(absTransforms.x, absTransforms.y);
            location = location.getTranslated(-thisAbsTransforms.x, -thisAbsTransforms.y);
            drawingElement._setLocation(location);
        }
    ...
    _setDrawingElementIndex: function (drawingElement, index, triggerEvents) {
        ...
        if (this.webgram && this.webgram.userInteraction()) {
    ...
    getDrawingElementById: function (id) {
        ...
        for (var i = 0; i < drawingElements.length; i++) {
            ...
            if (drawingElement.getDrawingElements) {
                drawingElement = drawingElement.getDrawingElementById(id);
                if (drawingElement) {
                    return drawingElement;
                }
            }
    ...
    _setWebgram: function (webgram) {
        Webgram.DrawingElements.ContainerElement.parent._setWebgram.call(this, webgram);
        
        Webgram.Utils.forEach(this._drawingElements, function (drawingElement) {
            drawingElement._setWebgram(webgram);
        });
    },
