...
Webgram.DrawingElements.ContainerElement.prototype = {
    ...
    setDrawingElements: function (drawingElements) {
        ...
        this._setTopLeft(new Webgram.Geometry.Point(minX, minY));
        this._setBottomRight(new Webgram.Geometry.Point(maxX, maxY));
        this._setTopLeft(new Webgram.Geometry.Point(minX, minY));
    ...
    _setConstraints: function () {
        ...
        for (var i = 0; i < this.drawingElements.length; i++) {
            ...
            for (var j = 0; j < shapeLen; j++) {
                ...
                var center = drawingElement.getCenter();
                if (minPoint != null && relPos != null) {
                    var width = (minPoint.x + center.x) / relPos.x;
                    var height = (minPoint.y + center.y) / relPos.y;
                    if (width > minWidth) {
                        minWidth = width;
                    }
                    if (height > minHeight) {
    ...
    _restoreChildrenRelativePositions: function () {
        ...
        for (var i = 0; i < this.drawingElements.length; i++) {
            var drawingElement = this.drawingElements[i];
            allChanged = allChanged && drawingElement.restoreParentRelativePosition();
        }
