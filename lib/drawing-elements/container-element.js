...
Webgram.DrawingElements.ContainerElement = 
    ...
    _addDrawingElement: function (drawingElement, where, triggerEvents) {
        ...
        var absLocation = null;
        var absRotationAngle = null;
        var absFlippedHorizontally = null;
        var absFlippedVertically = null;
        
        if (oldParent) {
            absLocation = drawingElement.getAbsLocation();
            absRotationAngle = drawingElement.getAbsRotationAngle();
            absFlippedHorizontally = drawingElement.isAbsFlippedHorizontally();
            absFlippedVertically = drawingElement.isAbsFlippedVertically();
        ...
        if (oldParent) {
            ...
            var thisAbsLocation = this.getAbsLocation();
            var thisAbsRotationAngle = this.getAbsRotationAngle();
            var thisAbsFlippedHorizontally = this.isAbsFlippedHorizontally();
            var thisAbsFlippedVertically = this.isAbsFlippedVertically();
            
            // TODO this will trigger geometry change events on elements that don't have various geometrical operations enabled
            if (absFlippedHorizontally !== Boolean(drawingElement.isFlippedHorizontally() ^ thisAbsFlippedHorizontally)) {
                drawingElement._flipHorizontally();
            }
            if (absFlippedVertically !== Boolean(drawingElement.isFlippedVertically() ^ thisAbsFlippedVertically)) {
                drawingElement._flipVertically();
            }
            drawingElement._setRotationAngle(drawingElement.getFlippedAngle(absRotationAngle - thisAbsRotationAngle));
            drawingElement._setLocation(absLocation.getTranslated(-thisAbsLocation.x, -thisAbsLocation.y));
        }
