...
Webgram.DrawingElements.ContainerElement.prototype = {
    ...
    _setShapePoint: function (index, point, transformed, snap) {
        Webgram.DrawingElements.RectangularElement.prototype._setShapePoint.call(this, index, point, transformed, snap);
        
        this._restoreChildrenRelativePositions();
    },
    ...
    setDrawingElements: function (drawingElements) {
        // TODO remove the existing drawing elements
        ...
        var snapToGridEnabled = this.isSnapToGridEnabled();
        this.setSnapToGridEnabled(false);
        this._setTopLeft(new Webgram.Geometry.Point(minX, minY), true);
        this._setBottomRight(new Webgram.Geometry.Point(maxX, maxY), true);
        this._setTopLeft(new Webgram.Geometry.Point(minX, minY), true);
        this.setSnapToGridEnabled(snapToGridEnabled);

        /* effectively add the drawing elements to the container element */
        for (i = 0; i < drawingElements.length; i++) {
            var drawingElement = drawingElements[i];
            
            drawingElement._enterContainerElement(this);
            drawingElement.setFocusType(Webgram.DrawingElement.FOCUS_NONE);
        }
    ...
    getMargins: function () {
        ...
        for (var i = 0; i < this.drawingElements.length; i++) {
            var drawingElement = this.drawingElements[i];
            boundingRectangle = drawingElement.getBoundingRectangle(true);
            if (boundingRectangle.x1 < minX) {
                minX = boundingRectangle.x1;
            }
            if (boundingRectangle.x2 > maxX) {
        ...
        if (minX === Infinity) { /* no children, margins are given by this block element */
            boundingRectangle = this.getBoundingRectangle(true);
            minX = boundingRectangle.x1;
            minY = boundingRectangle.y1;
            maxX = boundingRectangle.x2;
            maxY = boundingRectangle.y2;
    ...
    _setConstraints: function () {
        /* set the various constraints based on the children */
        ...
        for (var i = 0; i < this.drawingElements.length; i++) {
            ...
            if (drawingElement.isResizeEnabled && !drawingElement.isResizeEnabled()) {
                /* specific to RectangularElement instances */
                resizeEnabled = false;
            }
            
            var shapeLen = drawingElement.getShape().points.length;
            var centerRelPos = drawingElement._parentRelativePositions[shapeLen];
            var width, height;
            for (var j = 0; j < shapeLen; j++) {
                var relPos = drawingElement._parentRelativePositions[j];
                
                /* calculate the minimal size induced by the element */
                var minPoint = drawingElement.getMinShapePoint(j);
                minPoint = drawingElement.rotateDirect(drawingElement.scaleDirect(minPoint),
                        Webgram.Geometry.Point.zero());
                if (minPoint != null && relPos != null) {
                    width = minPoint.x / (relPos.x - centerRelPos.x) + 1;
                    height = minPoint.y / (relPos.y - centerRelPos.y) + 1;
                    if (width > minWidth) {
                        minWidth = width;
                    }
                    if (height > minHeight) {
                ...
                /* calculate the maximal size induced by the element */
//                var maxPoint = drawingElement.getMaxShapePoint(j); TODO bye bye
//                maxPoint = drawingElement.rotateDirect(drawingElement.scaleDirect(maxPoint),
//                        Webgram.Geometry.Point.zero());
//                if (maxPoint != null && relPos != null) {
//                    width = maxPoint.x / (relPos.x - centerRelPos.x) + 1;
//                    height = maxPoint.y / (relPos.y - centerRelPos.y) + 1;
//                    if (width < maxWidth) {
//                        maxWidth = width;
//                    }
//                    if (height < maxHeight) {
...
//                }
        ...
        this.setMoveEnabled(moveEnabled);
        this.setRotateEnabled(rotateEnabled);
        this.setResizeEnabled(resizeEnabled);
        
        this.setMinSize(new Webgram.Geometry.Size(minWidth, minHeight));
        //this.setMaxSize(new Webgram.Geometry.Size(maxWidth, maxHeight)); TODO bye bye
    },
    
    _setTop: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setTop.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setRight: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setRight.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setBottom: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setBottom.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setLeft: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setLeft.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setTopLeft: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setTopLeft.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setBottomRight: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setBottomRight.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setTopRight: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setTopRight.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setBottomLeft: function (point, snap, noFlip) {
        Webgram.DrawingElements.RectangularElement.prototype._setBottomLeft.call(this, point, snap, noFlip);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setWidth: function (width, snap) {
        Webgram.DrawingElements.RectangularElement.prototype._setWidth.call(this, width, snap);
        
        this._restoreChildrenRelativePositions();
    },
    
    _setHeight: function (height, snap) {
        Webgram.DrawingElements.RectangularElement.prototype._setHeight.call(this, height, snap);
    ...
    _restoreChildrenRelativePositions: function () {
        for (var i = 0; i < this.drawingElements.length; i++) {
            var drawingElement = this.drawingElements[i];
