...
Webgram.DrawingElements.GroupElement = Webgram.DrawingElements.RectangularElement.extend(
    ...
    // TODO jsdoc
    initialize: function GroupElement() {
        Webgram.DrawingElements.GroupElement.parentClass.call(this);
        ...
        this._fitChildrenLock = false; /* true during _fitChildren() calls */
        this._scaleChildrenLock = false; /* true during _scaleChildren() calls */
    },
    ...
    shapeFromJson: function (json) {
        Webgram.DrawingElements.GroupElement.parent.shapeFromJson.call(this, json);
     ...
     */
    getDrawingElementById: Webgram.DrawingElements.ContainerElement.prototype.getDrawingElementById,
    ...
    /**
     * Scales all the children by calling {@link Webgram.DrawingElement#fit},
     * according to their associated relative rectangular regions.
     */
    scaleChildren: function () {
        if (this._scaleChildrenLock) {
            return;
        }
    ...
    fitChildren: function () {
        ...
        if (delta) {
            ...
            Webgram.Utils.forEach(this._drawingElements, function (drawingElement) {
    ...
    _bindGeometryChangeHandlers: function (drawingElement) {
        drawingElement.onShapeChange.bind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onRotate.bind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onMove.bind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onFlipHorizontally.bind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onFlipVertically.bind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
    },
    
    _unbindGeometryChangeHandlers: function (drawingElement) {
        drawingElement.onShapeChange.unbind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onRotate.unbind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onMove.unbind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onFlipHorizontally.unbind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
        drawingElement.onFlipVertically.unbind(Webgram.DrawingElements.GroupElement._whenGeometryChanged);
    },
    ...
    duplicate: function (add, overrideJson) {
        /* we need to duplicate the children as well */
        
        var clone = Webgram.DrawingElements.GroupElement.parent.duplicate.call(this, add, overrideJson);
        
        clone._fitChildrenLock = true;
        
        var children = this.getDrawingElements();
        for (var i = 0; i < children.length; i++) {
            var child = children[i];
            child = child.duplicate(false);
            clone._addDrawingElement(child, null, false);
        }
        
        clone._fitChildrenLock = false;
        clone.fitChildren();
...
/** @lends Webgram.DrawingElements.GroupElement */ {
    ...
    _whenGeometryChanged: function () {
        /* "this" is the child */

        var groupElement = this._parent;
        
        /* avoid fitting to children when the change
         * is due to a change to the group element */
        if (groupElement._scaleChildrenLock || groupElement._fitChildrenLock) {
            return;
        }
        
        groupElement.fitChildren();
    }    
});
