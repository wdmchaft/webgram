...
Webgram.DrawingElements.PolyElement =
    ...
    /**
     * A base class for all the polygonal drawing elements.
     ...
     */
    addPoint: function (index, point, transformed) {
        if (!this._addPoint(index, point, transformed, true, true)) {
            return;
        }
    },
    
    _addPoint: function (index, point, transformed, snap, triggerEvents) {
        ...
        this._setShape(new Webgram.Geometry.Poly(points), snap);
     ...
     * (relative to the element)
     */
    setPoint: function (index, point, transformed) {
        this._setShapePoint(index, point, transformed, false);
        
        if (this.rotationCenterControlPoint) { // TODO is this still necessary?
    ...
    _addHoveredPoint: function () {
        ...
        this.addPoint(hoveredPointIndex, this._hoveredPoint, false);
    ...
    _getHoveredPointIndex: function () {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            if (Webgram.Geometry.pointInSegment(point, point1, point2, 3)) { /* a line tolerance of 3 should suffice */
    ...
    _computeHoveredPoint: function (mousePoint) {
        /* determine the segment that the point hovers */
        var points = this.getShape(false).points;
        for (var i = 0; i < points.length; i++) {
    ...
    pointInside: function (point) {
        var poly = this.getShape();
        
        if (poly.points.length <= 1) { /* a single point or nothing - either way the answer is no */
            return false;
        }
        
        if (!this.closed || poly.points.length < 3) {
    ...
    getLineTolerancePoly: function () {
        ...
        if (this.getShape().points.length === 0) {
            ...
            var point = this.getShapePoint(0, false);
            ...
            /* just two points */
            var p1 = this.getShapePoint(0, false);
            var p2 = this.getShapePoint(1, false);
    ...
    _snapShapePoint: function (index, point, fixedPoints, rotationAngle, center) {
        ...
        return this.super(index, point, fixedPoints, rotationAngle, center);
    },
    ...
    continueCreate: function (point, size, mouseDown, click) {
        ...
        if (click) {
            ...
            this.addPoint(this.getShape().points.length, point, true);
    ...
    shapeFromJson: function (json) {
        ...
        if (points.length > this.getShape().points.length) { /* points were added */
            for (i = 0; i < this.getShape().points.length; i++) {
                this._shape.points[i] = points[i];
            }
            
            for (i = this.getShape().points.length; i < points.length; i++) {
                this._addPoint(this.getShape().points.length, points[i], false, false);
            }
        }
            ...
            while (this.getShape().points.length > points.length) {
                this._remPoint(this.getShape().points.length - 1, false);
            }
        }
...
Webgram.DrawingElements.PolyElement.PolyControlPoint =
    ...
    computeAnchor: function () {
        return this.drawingElement.getShapePoint(this.polyPointIndex, false);
    },
    ...
    processMove: function (point, vanillaPoint) {
        this.drawingElement._setShapePoint(this.polyPointIndex, point, false, true);
        
        if (this.drawingElement.rotationCenterControlPoint) { // TODO needed?
            this.drawingElement.rotationCenterControlPoint.reset();
        }
        
        this.drawingElement.snap(this.polyPointIndex);// TODO proper snapping
