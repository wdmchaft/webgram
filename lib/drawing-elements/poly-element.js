    ...
    snapToAngleInternally: function (point, snapVisualFeedbackList, index) {
        ...
        if (prevPoint) {
            ...
            while (angle < Math.PI) {
                ...
                prevLines.push(Webgram.Geometry.Line.fromPointAndSlope(prevPoint, slope));
        ...
        if (nextPoint) {
            ...
            while (angle < Math.PI) {
                ...
                nextLines.push(Webgram.Geometry.Line.fromPointAndSlope(nextPoint, slope));
                ...
                if (dist < snapDistance) {
                    solutions.push({
                        ...
                        prevSnapVisualFeedback: new Webgram.DrawingElements.RootContainer.AngularSnapVisualFeedback(
                                this.transformDirect(inters), Math.atan(prevLine.slope)), // TODO this angle might not cover 2pi
                        nextSnapVisualFeedback: new Webgram.DrawingElements.RootContainer.AngularSnapVisualFeedback(
                                this.transformDirect(inters), Math.atan(nextLine.slope)) // TODO this angle might not cover 2pi
                    });
                }
            }
        }
        ...
        if (bestSolution) {
            snapVisualFeedbackList.push(new Webgram.DrawingElements.RootContainer.LineSnapVisualFeedback(
                    this.transformDirect(bestSolution)));
