...
/**
 * @class A store for JavaScript Image objects.
 * This class has a singleton object, {@link Webgram.imageStore}
 * that should be used when loading and drawing images with webgram.
 */
Webgram.ImageStore = function (webgram) {
    this._webgram = webgram;
    this._images = {};
};

Webgram.ImageStore.prototype = {
    /**
     ...
     * <tt>name</tt> and <tt>url</tt> as arguments
     */
    load: function (name, url, onLoad) {
        ...
        image.onload = function () {
            ...
            if (imageStore._webgram != null) {
                imageStore._webgram.invalidateDrawing();
                imageStore._webgram.invalidateDrawingMini();
            }
        };
    },
...
Webgram.Class('Webgram.ImageStore');
