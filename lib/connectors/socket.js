...
Webgram.Connectors.Socket.prototype = {
    ...
    computeAnchor: function () {
        var anchor = this._anchorFunc.call(this.drawingElement, this);
        
        return this.drawingElement.transformDirect(anchor);
    },
     ...
     * <em>(should not be overridden)</em>
     * @return {Number} the index of this socket
     */
    getIndex: function () {
    ...
    _dragAlongEndPoints: function () {
        /* (indirectly) move all endpoints connected to this socket */ 
        for (var i = 0; i < this._endPoints.length; i++) {
            var endPoint = this._endPoints[i];
            endPoint.movePoint(this.getAnchor());
            endPoint.drawingElement.invalidatePoints();
        }
    },
};
