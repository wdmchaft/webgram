...
Webgram.DrawingElement = Webgram.Class.extend( /** @lends Webgram.DrawingElement.prototype */ {
        ...
        if (snap && !this.isRotationCenterMoved() &&
            ...
            if (snappedAngle != null) {
                ...
                this.setSnapVisualFeedback(new Webgram.DrawingElements.RootContainer.AngularSnapVisualFeedback(
                        location, snappedAngle - Math.PI / 2));
     ...
     */
    snapExternally: function (point, snapVisualFeedbackList) {
        return this.snapToSiblings(point, snapVisualFeedbackList);
    },
     ...
     * either a point, a line or null, if no snapping was performed 
     */
    snapInternally: function (point, snapVisualFeedbackList) {
        return null;
    },
     ...
     * either a point, a line or null, if no snapping was performed 
     */
    snapToSiblings: function (point, snapVisualFeedbackList) {
        var snapDistance = this.getSetting('snapDistance');
        if (!snapDistance) {
            return null;
        }
        ...
        var siblings = this.getParent().getDrawingElements();
     ...
     * It also adds two extra fields to the returned point which
     * indicate what coordinates snapped and by which snapping method.
     * These fields are <tt>xSnapType</tt> and <tt>ySnapType</tt>
     * and may have the following values:<ul>
     *  <li><tt>internal</tt></li>, for internal snapping,
     *  <li><tt>external</tt></li>, for external snapping,
     *  <li><tt>grid</tt></li>, for snapping to grid or
     *  <li><tt>null</tt></li>, when no snapping was performed
     * </ul><br><br>
     * <em>(should not be overridden)</em>
     * @param {Webgram.Geometry.Point} the point to snap
     * @param {Boolean} internally whether to use internal snapping or not
     * @param {Boolean} externally whether to use external snapping or not
     * @param {Boolean} toGrid whether to use snapping to grid or not
    ...
    snapPoint: function (point, internally, externally, toGrid, visualFeedback) {
        ...
        var snappedInternally = internally && this.isSnapInternallyEnabled() && this.snapInternally(point, snapVisualFeedbackList) || null;
        var snappedExternally = externally && this.isSnapExternallyEnabled() && this.snapExternally(point, snapVisualFeedbackList) || null;
        var snappedToGrid = toGrid && this.isSnapToGridEnabled() && this.snapToGrid(point) || null;
        var tempPoint;
        ...
        if (snappedInternally) {
            if (snappedInternally instanceof Webgram.Geometry.Point) {
                ...
                snappedInternally = snappedInternally.clone();
                snappedInternally.xSnapType = 'internal';
                snappedInternally.ySnapType = 'internal';
                ...
                if (snappedExternally) {
                    ...
                    if (snappedExternally instanceof Webgram.Geometry.Point) {
                        ...
                        if (snappedInternally.isVertical()) {
                            tempPoint = snappedInternally.getPointByY(snappedExternally.y);
                            tempPoint.xSnapType = 'internal';
                            tempPoint.ySnapType = 'external';
                        }
                        else {
                            tempPoint = snappedInternally.getPointByX(snappedExternally.x);
                            tempPoint.xSnapType = 'external';
                            tempPoint.ySnapType = 'internal';
                        }
                        ...
                        if (intersection) {
                            intersection.xSnapType = 'internal';
                            intersection.ySnapType = 'internal';
                            ...
                            if (snappedInternally.isVertical()) {
                                ...
                                tempPoint.xSnapType = 'internal';
                                tempPoint.ySnapType = snappedToGrid ? 'grid' : null;
                            }
                            else {
                                tempPoint = snappedInternally.getPointByXs(tempPoint.x);
                                tempPoint.xSnapType = snappedToGrid ? 'grid' : null;
                                tempPoint.ySnapType = 'internal';
                            }
        ...
        if (snappedExternally) {
            if (snappedExternally instanceof Webgram.Geometry.Point) {
                ...
                snappedExternally = snappedExternally.clone();
                snappedExternally.xSnapType = 'external';
                snappedExternally.ySnapType = 'external';
                ...
                if (snappedExternally.isVertical()) {
        ...
        if (snappedToGrid) {
            snappedToGrid = snappedToGrid.clone();
            snappedToGrid.xSnapType = 'grid';
            snappedToGrid.ySnapType = 'grid';
        ...
        point = point.clone();
        point.xSnapType = null;
        point.ySnapType = null;
        
        return point;
    },
    
    _snapLocation: function () {
        /* changes the location such as the best
         * external snapping (or snapping to grid) is achieved */
        
        var minDeltaX = Infinity;
        var minDeltaY = Infinity;
        var minDeltaXGrid = Infinity;
        var minDeltaYGrid = Infinity;
        var snappedPointX = null;
        var snappedPointY = null;
        
        /* we independently gather the smallest snapping distance
         * for both axes and both external and to grid snapping methods */
        
        var snappingPoints = this.getSnappingPoints();
        for (var i = 0; i < snappingPoints.length; i++) {
            var snappingPoint = snappingPoints[i];
            var transformedSnappingPoint = this.transformDirect(snappingPoint);
            var snapped = this.snapPoint(snappingPoint, false, true, true, false);
            var transformedSnapped = this.transformDirect(snapped);
            
            if (snapped.xSnapType) {
                var deltaX = transformedSnapped.x - transformedSnappingPoint.x;
                
                if (snapped.xSnapType === 'grid') {
                    if (Math.abs(deltaX) < Math.abs(minDeltaXGrid)) {
                        minDeltaXGrid = deltaX;
                    }
                }
                else /* if (snapped.xSnapType === 'external') */ {
                    if (Math.abs(deltaX) < Math.abs(minDeltaX)) {
                        minDeltaX = deltaX;
                        snappedPointX = transformedSnapped.x;
                    }
                }
            }
            
            if (snapped.ySnapType) {
                var deltaY = transformedSnapped.y - transformedSnappingPoint.y;
                
                if (snapped.ySnapType === 'grid') {
                    if (Math.abs(deltaY) < Math.abs(minDeltaYGrid)) {
                        minDeltaYGrid = deltaY;
                    }
                }
                else /* if (snapped.ySnapType === 'external') */ {
                    if (Math.abs(deltaY) < Math.abs(minDeltaY)) {
                        minDeltaY = deltaY;
                        snappedPointY = transformedSnapped.y;
                    }
                }
            }
        }
        
        /* external snapping takes precedence over snapping to grid;
         * snapping to grid doesn't display visual feedback */
        
        if (!isFinite(minDeltaX)) {
            minDeltaX = minDeltaXGrid;
            snappedPointX = null;
        }
        if (!isFinite(minDeltaX)) {
            minDeltaX = 0;
        ...
        /* display a snapping visual feedback */
        var snapVisualFeedback = null;
        if (snappedPointX != null) {
            if (snappedPointY != null) {
                /* both axes snapped, use a point snapping visual feedback */
                snapVisualFeedback = new Webgram.DrawingElements.RootContainer.PointSnapVisualFeedback(
                        new Webgram.Geometry.Point(snappedPointX, snappedPointY));
            }
            else {
                /* only x axis snapped, use a line snapping visual feedback */
                snapVisualFeedback = new Webgram.DrawingElements.RootContainer.LineSnapVisualFeedback(
                        new Webgram.Geometry.Line(Infinity, snappedPointX));
            }
        }
        else {
            if (snappedPointY != null) {
                /* only y axis snapped, use a line snapping visual feedback */
                snapVisualFeedback = new Webgram.DrawingElements.RootContainer.LineSnapVisualFeedback(
                        new Webgram.Geometry.Line(0, snappedPointY));
            }
            /* else, no snap visual feedback */
        }
        
        if (snapVisualFeedback) {
            this.setSnapVisualFeedback(snapVisualFeedback);
        }
        
        this._location = this._location.getTranslated(minDeltaX, minDeltaY);
    },
    ...
    /**
     * Determines if a <tt>point</tt> is inside of one of this element's sockets' area or not.
     * @param {Webgram.Geometry.Point} point the point to test, relative to the element's parent (i.e. transformed)
    ...
    updateDependentElements: function () {
        ...
        for (var i = 0; i < this._sockets.length; i++) {
            var socket = this._sockets[i];
            socket._dragAlongEndPoints(); // TODO this is not the right way to do it
        }
