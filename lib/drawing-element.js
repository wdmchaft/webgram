...
Webgram.DrawingElement = Webgram.Class.extend( /** @lends Webgram.DrawingElement.prototype */ {
     ...
     * These fields are <tt>xSnapType</tt> and <tt>ySnapType</tt> TODO update jsdoc here
    ...
    _snapLocation: function () {
        ...
        // TODO if rotation angle is small (<30deg) snapping to a vertical line makes the component very jumpy vertically
        ...
        for (var i = 0; i < snappingPoints.length; i++) {
            ...
            if (snappedExternally instanceof Webgram.Geometry.Point) {
                pointSolutions.push({
                    ...
                    snappedPoint: snappedExternally,
        ...
        for (i = 0; i < lineSolutions.length - 1; i++) {
            for (var j = i + 1; j < pointSolutions.length; j++) {
        ...
        /* finally, if no final solutions were found yet,
        ...
        if (pointSolutions.length === 0 && lineSolutions.length > 0 && this.isSnapToGridEnabled()) {
            for (i = 0; i < lineSolutions.length; i++) {
                ...
                var point;
                if (lineSolution.snappedLine.isVertical()) {
                ...
                pointSolutions.push({
                    snappingPoint: lineSolution.snappingPoint,
                    snappedPoint: point,
        ...
        // TODO if no line and no point solutions, add solutions grid snappings of each snapping point
        ...
        if (bestSolution) {
            ...
            this._location = this._location.getTranslated(size.width, size.height);
        }
    },
