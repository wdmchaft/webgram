...
Webgram.DrawingElement.prototype = {
     ...
     * thus making its rotation relative to this element.
     * This method does not affect the original object.
     * @see Webgram.Geometry
     * @param {Object} geometry the geometric object to rotate
     * @returns {Object} the geometric object relative to this element 
     ...
     * thus making its coordinates relative to the parent's coordinate system.
     * This method does not affect the original object.
     * @see Webgram.Geometry
     ...
     * thus making its coordinates relative to this element.
     * This method does not affect the original object.
     * @see Webgram.Geometry
     * @param {Object} geometry the geometric object to scale
     * @returns {Object} the geometric object relative to this element 
    ...
    snap: function (index) {
        ...
        if (index == null) { /* snap the whole element */
            ...
            this._setTransformedShapePoint(index, rotatedShape.points[index], true);
        }
    },
    ...
    _applyShapePointConstraints: function (index, point, fixedPoints, rotationAngle, center) {
        ...
        // TODO this method does not apply the min/max constraints properly (regardless of the center)
        
        /* compute the new center */
        fixedPoints = fixedPoints.slice();
        fixedPoints.push(point);
        var newCenter = new Webgram.Geometry.Poly(fixedPoints).getCenter();
        
        point = point.getTranslated(-newCenter.x, -newCenter.y);
        
        /* apply the minimum constraints */
        var minPoint = this.getMinShapePoint(index);
        if (minPoint != null) {
            if ((minPoint.x < 0 && point.x > minPoint.x) || (minPoint.x > 0 && point.x < minPoint.x)) {
                point = new Webgram.Geometry.Point(minPoint.x, point.y);
            }
            if ((minPoint.y < 0 && point.y > minPoint.y) || (minPoint.y > 0 && point.y < minPoint.y)) {
                point = new Webgram.Geometry.Point(point.x, minPoint.y);
            }
        }
        ...
        if (maxPoint != null) {
            if ((maxPoint.x < 0 && point.x < maxPoint.x) || (maxPoint.x > 0 && point.x > maxPoint.x)) {
                point = new Webgram.Geometry.Point(maxPoint.x, point.y);
            }
            if ((maxPoint.y < 0 && point.y < maxPoint.y) || (maxPoint.y > 0 && point.y > maxPoint.y)) {
                point = new Webgram.Geometry.Point(point.x, maxPoint.y);
            }
        }
