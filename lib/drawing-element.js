...
Webgram.DrawingElement = Webgram.Class.extend( /** @lends Webgram.DrawingElement.prototype */ {
    ...
    _setLocation: function (location, snap, partial) {
        ...
        if (snap) {
            location = this._getSnappedLocation(location);
        }
     ...
     */
    snapExternally: function (point) {
        return this._snapToSiblings(point);
    },

    /**
     * Snaps the given point <em>internally</em> (i.e. against other parts of this element). By default,
     * no snapping is performed and the method returns <tt>null</tt>.<br><br>
     * <em>(could be overridden)</em>
     * @param {Wegram.Geometry.Point} point the point to snap
     * @returns {Webgram.Geometry.Point|Webgram.Geometry.Line} the snapped geometry,
     * either a point, a line or null, if no snapping was performed 
     */
    snapInternally: function (point) {
        // TODO implement me
    },
    ...
    _snapToSiblings: function (point) {
        /* snaps a given point to each of the
         * snapping points of the siblings,
         * returning the best snapping result;
         * 
         * the result is either a point, a line or null */
        ...
        for (var i = 0; i < siblings.length; i++) {
            ...
            for (var j = 0; j < snappingPoints.length; j++) {
                ...
                if (distX < minDistX && distX < snapDistance) {
                    minDistX = distX;
                    closestPointX = snappingPoint.x;
                }
                if (distY < minDistY && distY < snapDistance) {
                    minDistY = distY;
                    closestPointY = snappingPoint.y;
                }
            }
        }
        ...
        if (closestPointX != null) {
            if (closestPointY != null) {
                ...
                return this.transformInverse(new Webgram.Geometry.Point(closestPointX, closestPointY));
            }
            else {
                /* snapping only along x */
                
                return this.transformInverse(new Webgram.Geometry.Line(Infinity, closestPointX));
            }
        }
        else {
            if (closestPointY != null) {
                ...
                return this.transformInverse(new Webgram.Geometry.Line(0, closestPointY));
            }
            else {
                /* no snapping at all */
    ...
    _getSnappedLocation: function () {
        /* computes the location such as the best
        ...
        var zero = Webgram.Geometry.Point.zero();
        var snappingPoint;
        var snappingPoints = this.getSnappingPoints();
        
        if (this.isSnapExternallyEnabled()) {
            for (var i = 0; i < snappingPoints.length; i++) {
                snappingPoint = snappingPoints[i];
                var snapped = this.snapExternally(snappingPoint);
                if (!snapped) {
                    continue;
                }
                
                if (snapped instanceof Webgram.Geometry.Point) {
                    var delta = new Webgram.Geometry.Size(snapped.x - snappingPoint.x, snapped.y - snappingPoint.y);
                    delta = this.rotateDirect(this.scaleDirect(delta), zero);
                    
                    if (Math.abs(delta.width) < Math.abs(minDeltaX)) {
                        minDeltaX = delta.width;
                        closestPointX = this.transformDirect(snapped).x;
                    }
                    if (Math.abs(delta.height) < Math.abs(minDeltaY)) {
                        minDeltaY = delta.height;
                        closestPointY = this.transformDirect(snapped).y;
                    }
                }
                else { /* if snapped instanceOf Webgram.Geometry.Line */
                    if (snapped.isHorizontal()) {
                        var delta = new Webgram.Geometry.Size(0, snapped.intercept - snappingPoint.y);
                        delta = this.rotateDirect(this.scaleDirect(delta), zero);
                        
                        if (Math.abs(delta.height) < Math.abs(minDeltaY)) {
                            minDeltaY = delta.height;
                            closestPointY = this.transformDirect(new Webgram.Geometry.Point(0, snapped.intercept)).y;
                        }
                    }
                    else if (snapped.isVertical()) {
                        var delta = new Webgram.Geometry.Size(snapped.intercept - snappingPoint.x, 0);
                        delta = this.rotateDirect(this.scaleDirect(delta), zero);
                        
                        if (Math.abs(delta.width) < Math.abs(minDeltaX)) {
                            minDeltaX = delta.width;
                            closestPointX = this.transformDirect(new Webgram.Geometry.Point(snapped.intercept, 0)).x;
                        }
                    }
                    else {
                        /* no oblique line external snapping */
                    }
                }
            }
        }
        
        var xSnappedToGrid = false;
        var ySnappedToGrid = false;
        
        /* snap to grid, if needed and enabled */
        if (this.isSnapToGridEnabled() && this.getSetting('snapGrid') != null && (closestPointX == null || closestPointY == null)) {
            var xSnapped = (closestPointX != null);
            var ySnapped = (closestPointY != null);
            
            for (var i = 0; i < snappingPoints.length; i++) {
                snappingPoint = snappingPoints[i];
                if (snappingPoint.equals(zero)) {
                    continue; /* the origin is not included in the snapping algorithm */
                }
                
                var snapped = this.snapToGrid(snappingPoint);
                var delta = new Webgram.Geometry.Size(snapped.x - snappingPoint.x, snapped.y - snappingPoint.y);
                delta = this.rotateDirect(this.scaleDirect(delta), zero);
                
                if (!xSnapped && Math.abs(delta.width) < Math.abs(minDeltaX)) {
                    minDeltaX = delta.width;
                    closestPointX = this.transformDirect(snapped).x;
                    xSnappedToGrid = true;
                }
                if (!ySnapped && Math.abs(delta.height) < Math.abs(minDeltaY)) {
                    minDeltaY = delta.height;
                    closestPointY = this.transformDirect(snapped).y;
                    ySnappedToGrid = true;
                }
        ...
        this.setSnapVisualFeedback({
            type: 'linear',
            x: xSnappedToGrid ? null : closestPointX,
            y: ySnappedToGrid ? null : closestPointY,
        });
        
        minDeltaX = isFinite(minDeltaX) ? minDeltaX : 0;
        minDeltaY = isFinite(minDeltaY) ? minDeltaY : 0;
        
        return this._location.getTranslated(minDeltaX, minDeltaY);
    },
