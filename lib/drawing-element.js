...
Webgram.DrawingElement.prototype = {
    ...
    finishStyleEvents: function (force) {
        ...
        if (this._styleChanged) {
            ...
            this._finishDependentEvents(); /* call finish*Events on dependent DEs */
        }
    },
...
//                    }
    ...
    finishRotationAngleEvents: function (force) {
        if (force && !this._rotationAngleChanged) {
            this._rotationAngleChanged = true;
            this.onBeginRotate.trigger();
            ...
            this.onRotate.trigger(this.getRotationAngle());
        }
    ...
    flipShapeHorizontally: function () {
        ...
        for (var i = 0; i < this.getShape().points.length; i++) {
            var point = new Webgram.Geometry.Point(2 * center.x - shape.points[i].x, shape.points[i].y);
            points.push(point);
        }
    ...
    flipShapeVertically: function () {
        ...
        var shape = this.getShape();
        for (var i = 0; i < this.getShape().points.length; i++) {
            var point = new Webgram.Geometry.Point(shape.points[i].x, 2 * center.y - shape.points[i].y);
            points.push(point);
        }
     ...
     * as invalid, forcing future calls to their respective {@link Webgram.ControlPoint#update} methods.
     */
    ...
    _finishDependentEvents: function () {
        /* calls finish*Events() on elements that depend on this one */
        
        /* first treat the elements connected
         * through connectors */
        var i, j, drawingElement;
        for (i = 0; i < this._sockets.length; i++) {
            var socket = this._sockets[i];
            for (j = 0; j < socket._endPoints.length; j++) {
                var endPoint = socket._endPoints[j];
                drawingElement = endPoint.drawingElement;
                
                drawingElement.finishChangeEvents();
            }
        }
        
        /* then treat elements that were explicitly marked
         * as being dependent on this one */
        for (i = 0; i < this._dependentDrawingElements.length; i++) {
            drawingElement = this._dependentDrawingElements[i];
