...
Webgram.DrawingElement = function (id, points) {
    ...
    /**
     * An event that is triggered whenever one of the following attributes begins to change:<br>
     *  * shape<br>
     *  * center, rotation angle, flip state<br>
     *  * stroke style, fill style, text style<br>
     *  * z-index<br>
    ...
    /**
     * An event that is triggered whenever one of the following attributes changes:<br>
     *  * shape<br>
     *  * center, rotation angle, flip state<br>
     *  * stroke style, fill style, text style<br>
     *  * z-index<br>
    ...
    /**
     * An event that is triggered whenever one of the following attributes finishes to change:<br>
     *  * shape<br>
     *  * center, rotation angle, flip state<br>
     *  * stroke style, fill style, text style<br>
     *  * z-index<br>
...
Webgram.DrawingElement.prototype = {
    ...
    drawText: function (text, box, textStyle) {
        if (box == null) {
            // TODO to be replaced with an untransformed getBaseRectangle
     ...
     * The rectangle is relative to this element, and has the size dictated by the
     ...
     * will be enlarged (or shrinked) according to the {@link Webgram.DrawingElement#getDecorationOffset} method.<br><br>
     * <em>(could be overridden)</em>
    ...
    getDecorationPoly: function () {
        ...
        decorationRect = decorationRect.getShrinked(-offset);
     ...
     * @see Webgram.DrawingElement#setShapePoint
     * @see Webgram.DrawingElement#setTransformedShapePoint
     ...
     * We call the polygon made of these points the <em>shape</em> of the element.<br>
     * Shape points are untransformed (relative to the center of the element).<br>
     * In most cases you don't really have to deal with this field, but rather use the following methods:<br> 
     * {@link Webgram.DrawingElement#setTransformedShapePoint},<br>
     * {@link Webgram.DrawingElement#setShapePoint},<br>
...
//            }
     ...
     * all the <em>untransformed</em> shape points.
     ...
     * rectangle of the transformed points, otherwise returns the bounding
     * rectangle of the untransformed points
     ...
     * @param {Boolean} transformed set to <tt>true</tt> if the point is transformed
     * (relative to the parent), or <tt>false</tt> if the point is untransformed
    ...
    pointInside: function (point, transformed) {
        ...
        if (transformed) {
            shape = this.transformDirect(shape);
        }
    ...
    snap: function (index) {
        ...
        if (index == null) { /* snap the whole element */
            ...
            if (minDeltaX < Infinity) {
                this.setShape(this.getShape().getTranslated(minDeltaX, 0));
            }
            if (minDeltaY < Infinity) {
                this.setShape(this.getShape().getTranslated(0, minDeltaY));
            }
            ...
            this._setTransformedShapePoint(index, transformedShape.points[index], true);
        }
    },
    ...
    restoreParentRelativePosition: function () {
        ...
        for (var i = 0; i < shape.points.length; i++) {
            ...
            if (this.setTransformedShapePoint(i, point)) {
                changed = true;
            }
        }
...
//        }
    ...
    invalidate: function (mini) {
        ...
        if (this.webgram) {
            this.webgram.invalidateDrawing();
            if (mini) {
                this.webgram.invalidateDrawingMini();
            }
        }
    },
