...
Webgram.DrawingElement.prototype = {
    ...
    getBounds: function () {
        ...
        var boundingPoly = this.transformDirect(poly);
     ...
     * We call the polygon made of these points the <em>shape</em> of the element.<br>
     * Shape points are relative to the parent and are not affected by the element's rotation angle.<br>
     * @param {Number} rotated set to <tt>true</tt> to return the rotated shape
     * @param {Number} scaled set to <tt>true</tt> to return the scaled (flipped) shape
     * @see Webgram.DrawingElement#setRotatedShapePoint
     ...
     */
    getShape: function (rotated, scaled) {
        var shape;
        if (scaled) {
            shape = this.translateDirect(this.scaleDirect(this.translateInverse(this._shape)));
        }
        else {
            shape = this._shape;
        }
    ...
    /**
     * Returns a point of the shape polygon of this element.
     * @param {Number} index the index of the point to return
     * @param {Number} rotated set to <tt>true</tt> to return the rotated shape point
     * @param {Number} scaled set to <tt>true</tt> to return the scaled (flipped) shape point
     * @see Webgram.DrawingElement#getShape
     * @returns {Webgram.Geometry.Point} the point
     */
    getShapePoint: function (index, rotated, scaled) {
        var point;
        if (scaled) {
            point = this.translateDirect(this.scaleDirect(this.translateInverse(this._shape.points[index])));
        }
        else {
            point = this._shape.points[index];
        }
...
//                    }
            ...
            var rotatedShape = this.getShape(true, true);
    ...
    getSnappingPoints: function () {
        var points = this.getShape(true, true).points;
    ...
    _setRotatedShapePoint: function (index, point, noSnapFeedback) {
        ...
        for (var i = 0; i < this.getShape().points.length; i++) {
            if (i !== index) {
                nrFixedPoints.push(this.getShapePoint(i)); 
                fixedPoints.push(this.getShapePoint(i).getRotated(rotationAngle, oldCenter));
            }
        }
