...
Webgram.DrawingElement = function (id, points) {
    ...
    /**
     * An event that is triggered when the user begins changing one of the styles.<br>
     * Handlers receive no arguments. 
     * @type Webgram.Event
     */
    this.onBeginStyleChange = new Webgram.Event('begin style change', this); /* () */

    /**
     * An event that is triggered whenever one of the styles of this element changes.<br>
     * Handlers receive no arguments. 
     * @type Webgram.Event
     */
    this.onStyleChange = new Webgram.Event('style change', this); /* () */

    /**
     * An event that is triggered when the user finishes changing one of the styles.<br>
     * Handlers receive no arguments. 
     * @type Webgram.Event
     */
    this.onEndStyleChange = new Webgram.Event('end style change', this); /* () */
     ...
     * @type Webgram.Event
     */
    this.onEndShapeChange = new Webgram.Event('shape end change', this); /* () */
    
    /**
     * An event that is triggered after the style, the shape
     * or the rotation angle of this drawing element is modified.
     * In addition to these, any additional attributes that contribute
     * to the json form of the element should trigger this event.
...
Webgram.DrawingElement.prototype = {
     ...
     *  also be redrawn.
     */
    invalidateDrawing: function (mini) {
        if (this.webgram) {
    ...
    getDrawingPoly: function () {
        return this.translateInverse(this.getShape());
    },
    ...
    setStrokeStyle: function (strokeStyle) {
        this._setStrokeStyle(strokeStyle, true);
    ...
    setFillStyle: function (fillStyle) {
        this._setFillStyle(fillStyle, true);
    ...
    setTextStyle: function (textStyle) {
        this._setTextStyle(textStyle, true);
    ...
    setGuidesStyle: function (strokeStyle) {
        // TODO use the same mechanism as for setStrokeStyle()
        
        this.beginStyleChange();
        this._guidesStyle = strokeStyle;
        
        this.onStyleChange.trigger();
    ...
    /**
     * Call this method before changing any property
     * that is part of the style of this element.
     * Calling this method will invalidate the element,
     * create an undo check point and trigger the corresponding events.
     */
    beginStyleChange: function () {
        if (!this._styleChanged) {
            this._styleChanged = true;
            this.onBeginStyleChange.trigger();
     ...
     * one of the styles has been changed:
     * {@link Webgram.DrawingElement#onBeginStyleChange},
     * {@link Webgram.DrawingElement#onStyleChange} and 
     * {@link Webgram.DrawingElement#onEndStyleChange}.
     * This method assures a successive and uniform triggering
     * of the three events. It's automatically invoked when
     * one of the style is modified via the library-provided
     * methods {@link Webgram.DrawingElement#setStrokeStyle}, 
     * @link Webgram.DrawingElement#setFillStyle} or @link Webgram.DrawingElement#setTextStyle}.
     * @param {Boolean} force set to <tt>true</tt> to force the triggering of the events,
     *  regardless of the fact that a style was actually changed or not.
     */
    finishStyleEvents: function (force) {
        if (force) {
            this.beginStyleChange();
        }
        ...
        if (this._styleChanged) {
            ...
            this._styleChanged = false;
            this.onEndStyleChange.trigger();
            
            this._finishDependentChangeEvents(); /* call finish*Events on dependent DEs */
        }
    },
    ...
    getBoundingRectangle: function () {
        return this.getShape().getRotated(this.getRotationAngle(), this.getCenter()).getBoundingRectangle();
    },
    
    /**
     * Builds and object with the boundaries of this element.
     ...
     * </ul>
     */
    getBounds: function (transform) {
        var poly = this.translateInverse(this.getBaseRectangle()).getPoly();
        if (transform) {
            poly = this.transformDirect(poly);
        }
        
        return {
            topLeft: poly.points[0],
            top: poly.points[0].getCenterTo(poly.points[1]),
            topRight: poly.points[1],
            right: poly.points[1].getCenterTo(poly.points[2]),
            bottomRight: poly.points[2],
            bottom: poly.points[2].getCenterTo(poly.points[3]),
            bottomLeft: poly.points[3],
            left: poly.points[3].getCenterTo(poly.points[0])
        };
    },
     ...
     * Shape points are translated so that they are relative to the parent <b>but</b>
     * they are not affected by the element's rotation angle or flipping state,
     * unless <tt>transformed</tt> is set to <tt>true</tt>.<br>
     ...
     * @see Webgram.DrawingElement#getBoundingRectangle
     * @see Webgram.DrawingElement#getCenter
     * @see Webgram.DrawingElement#getWidth
     * @see Webgram.DrawingElement#getHeight
     * @returns {Webgram.Geometry.Poly} the shape of this element
     */
    getShape: function (transformed) {
        if (transformed) {
            return this.transformDirect(this.translateInverse(this._shape));
        }
        else {
            return this._shape;
        }
     ...
     * Each drawing element has a set of one or more points that serve as anchors and often give its exact shape.
     * We call the polygon made of these points the <em>shape</em> of the element.<br>
     * Shape points are relative to the parent and are not affected by the element's rotation angle.<br>
     * In most cases you don't really have to deal with this field, but rather use methods like<br> 
    ...
    setShape: function (shape) {
        ...
        /* trigger the base rectangle mechanism,
         * to update all dependent elements */
    ...
    getShapePoint: function (index, transformed) {
        if (transformed) {
            return this.transformDirect(this.translateInverse(this._shape.points[index]));
        }
        else {
     ...
     */
    setShapePoint: function (index, point) {
        this.beginShapeChange();
    ...
    /**
     * Modifies the <tt>index</tt>-th point of the shape so that when applying the transforms,
     * the final point will have the coordinates specified by <tt>point</tt>, relative to the parent.<br>
     * The other points of the shape will be recalculated in such a way that when transformed,
     * they will keep their position relative to the parent.<br>
     * This method does not apply any kind of snapping.<br><br>
     * <em>(should not be overridden)</em>
     * @param {Number} index the index of the shape point to be modified
     * @param {Webgram.Geometry.Point} point the new desired position of the point
     * @returns {Boolean} <tt>true</tt> if the point was actually changed,
     * <tt>false</tt> otherwise (e.g. not changed due to constraints)
     */
    setTransformedShapePoint: function (index, point) {
        return this._setTransformedShapePoint(index, point, true);
    },
    ...
    /**
     * Call this method before changing any property
     * that is part of the shape of this element.
     * Calling this method will invalidate the element,
     * create an undo check point and trigger the corresponding events.
     */
    beginShapeChange: function () {
        ...
        this._baseRectangle = null; /* invalidate the base rectangle */
    ...
    setRotationAngle: function (angle) {
        this._setRotationAngle(angle, true);
    },
    ...
    /**
     * Call this method before changing the rotation
     * angle of this element.
     * Calling this method will invalidate the element,
     * create an undo check point and trigger the corresponding events.
     */
    beginRotationAngleChange: function () {
    ...
    /**
     * A drawing element uses three events to signal that
     * the rotation angle has been changed:
     * {@link Webgram.DrawingElement#onBeginRotate},
     * {@link Webgram.DrawingElement#onRotate} and 
     * {@link Webgram.DrawingElement#onEndRotate}.
     * This method assures a successive and uniform triggering
     * of the three events. It's automatically invoked when
     * the rotation angle is modified via the library-provided
     * method {@link Webgram.DrawingElement#setRotationAngle}.
     * @param {Boolean} force set to <tt>true</tt> to force the triggering of the events,
     *  regardless of the fact that the rotation angle was actually changed or not.
     */
    finishRotationAngleEvents: function (force) {
        if (force) {
            this.beginRotationAngleChange();
        }
     ...
     * as invalid, telling webgram to update and redraw it as soon as possible.
     */
    invalidate: function () {
        for (var i = 0; i < this._controlPoints.length; i++) {
    ...
    shapeToJson: function () {
        ...
        if (this._parent && (this._parent instanceof Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer)) {
            ...
            if (msceParent.getRotationAngle() !== 0) {
                ...
                shape = this.getShape().getTranslated(deltaX, deltaY);
            }
            else {
                shape = this.getShape().getTranslated(msceCenter.x, msceCenter.y);
            }
        }
        else {
            shape = this.getShape();
    ...
    shapeFromJson: function (json) {
        ...
        points = [];
        for (i = 0; i < shape.points.length; i++) {
            var fixedPoints = shape.points.slice(0, i).concat(shape.points.slice(i + 1));
            points.push(this._applyShapePointConstraints(i, shape.points[i], fixedPoints, rotationAngle, center));
        }
    ...
    strokeStyleFromJson: function (json) {
        if (json !== null) {
            ...
            this._setStrokeStyle(null, true);
        }
    },
    ...
    finishChangeEvents: function () {
        /* workaround to avoid triggering the onChange
         * event more than one time */
        var oldOnChange = this.onChange;
        this.onChange = new  Webgram.Event('dummy', this);
        var triggered = false;
        this.onChange.bind(function () {
            triggered = true;
        });
        ...
        this.finishShapeEvents();
        this.finishRotationAngleEvents();
        this.finishStyleEvents();
    ...
    _setStrokeStyle: function (strokeStyle, triggerEvents) {
        this.beginStyleChange();
    ...
    _setFillStyle: function (fillStyle, triggerEvents) {
        this.beginStyleChange();
    ...
    _setTextStyle: function (textStyle, triggerEvents) {
        this.beginStyleChange();
    ...
    _setTransformedShapePoint: function (index, point, noSnapFeedback) {
        ...
        for (var i = 0; i < shape.points.length; i++) {
            if (i !== index) {
                nrFixedPoints.push(shape.points[i]);
                fixedPoints.push(this.getShapePoint(i, true, true));
            }
        }
        
        /* compute the new unrotated, unscaled point */
        var nrPoint = this.translateDirect(this.transformInverse(point));
        ...
        /* compute the new rotated, scaled point */
        nrPoint = nrPoint.getTranslated(-oldCenter.x, -oldCenter.y);
        nrPoint = this.scaleDirect(nrPoint);
        nrPoint = nrPoint.getTranslated(oldCenter.x, oldCenter.y);
        ...
        /* determine the new unrotated & unscaled points */
        poly = new Webgram.Geometry.Poly(fixedPoints).
                getRotated(-rotationAngle, newCenter).
                getTranslated(-newCenter.x, -newCenter.y);
        poly = this.scaleInverse(poly);
        poly = poly.getTranslated(newCenter.x, newCenter.y);
        
        nrFixedPoints = poly.points;
        point = point.getRotated(-rotationAngle, newCenter).getTranslated(-newCenter.x, -newCenter.y);
        point = this.scaleInverse(point);
        nrPoint = point.getTranslated(newCenter.x, newCenter.y);
        
        /* restore all shape points */
        nrFixedPoints.splice(index, 0, nrPoint);
        var changed = !nrPoint.equals(this.getShapePoint(index));
        this.setShape(new Webgram.Geometry.Poly(nrFixedPoints));
    ...
    _finishDependentChangeEvents: function () {
    ...
    _setRotationAngle: function (angle, triggerEvents) {
        ...
        if (this.isSnapToAngleEnabled() && snapAngle != null && 
                !this.isRotationCenterMoved() && /* only snap when rotation center is not moved*/
                snapAngle != null) {
