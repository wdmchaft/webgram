...
Webgram.DrawingElement = Webgram.Class.extend( /** @lends Webgram.DrawingElement.prototype */ {
    ...
    /**
     * Returns the absolute (apparent) rotation angle of this element.
     * @returns {Number} the absolute rotation angle
     */
    getAbsRotationAngle: function () {
        var origin = Webgram.Geometry.Point.zero();
        var dummyPoint = origin.getPointAt(100, this._rotationAngle);
        
        var drawingElement = this._parent;
        while (drawingElement && drawingElement._parent) {
            origin = drawingElement.transformDirect(origin);
            dummyPoint = drawingElement.transformDirect(dummyPoint);
            
            drawingElement = drawingElement._parent;
        }

        return origin.getAngleTo(dummyPoint);
    },
    ...
    /**
     * Returns the absolute horizontal flipping state of this element.
     * @returns {Boolean} the absolute horizontal flipping state
     */
    isAbsFlippedHorizontally: function () {
        var flippedHorizontally = this.isFlippedHorizontally();
        
        var drawingElement = this;
        while (drawingElement._parent) {
            flippedHorizontally = Boolean(flippedHorizontally ^ drawingElement._parent.isFlippedHorizontally());
    ...
    /**
     * Returns the absolute vertical flipping state of this element.
     * @returns {Boolean} the absolute vertical flipping state
     */
    isAbsFlippedVertically: function () {
        var flippedVertically = this.isFlippedVertically();
        
        var drawingElement = this;
        while (drawingElement._parent) {
            flippedVertically = Boolean(flippedVertically ^ drawingElement._parent.isFlippedVertically());
