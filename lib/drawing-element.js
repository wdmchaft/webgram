...
Webgram.DrawingElement = Webgram.Class.extend( /** @lends Webgram.DrawingElement.prototype */ {
     ...
     */
    initialize: function DrawingElement() {
        /**
        ...
        /**
         * An event that is triggered whenever the element is restored from a <em>json</tt> object.<br>
         * Handlers receive the following arguments: <tt>(json)</tt>.
         * @type Webgram.Event
         */
        this.onRestore = new Webgram.Event('restore', this); /* (json) */
        ...
        /**
         * An event that is triggered when the element is duplicated.
         * The event is triggered on both the original and the clone.<br>
         * Handlers receive no arguments. 
         * @type Webgram.Event
         */
         ...
         * by a call to {@link Webgram.DrawingElement#setFocus}.<br>
         * Handlers receive no arguments. 
         * @type Webgram.Event
         */
        ...
        this._focus = Webgram.DrawingElement.FOCUS_NONE;
        this._jsonFields = [];
        this._jsonFuncs = [];
    ...
    drawNoZoomTop: function() {
        ...
        if (this.getFocus() === Webgram.DrawingElement.FOCUS_SELECTED || this.getFocus() === Webgram.DrawingElement.FOCUS_HOVERED) {
            this.drawGuides();
        }
    },
    ...
    drawDecoration: function () {
        switch (this.getFocus()) {
            ...
            case Webgram.DrawingElement.FOCUS_SELECTED_MULTIPLE: // TODO remove this
                if (this._selectedDecorationStyle) {
                    this.drawPoly(this.getDecorationPoly(), true);
                    this.paint(this._selectedDecorationStyle.replace({colors: ['red']}), null);
                }
    ...
    drawGuides: function () {
        ...
        if (this._gradientControlPoints != null && this.getFocus() === Webgram.DrawingElement.FOCUS_SELECTED) {
    ...
    drawControlPoints: function () {
        if ((this._focus === Webgram.DrawingElement.FOCUS_HOVERED && this.isHoveredControlPointsEnabled()) ||
             this._focus === Webgram.DrawingElement.FOCUS_SELECTED) {
            
            Webgram.Utils.forEach(this._controlPoints, function (controlPoint) {
                controlPoint.draw();
            });
        }
    },
    ...
    drawLine: function (point1, point2) {
        if (this.webgram.rootContainer.isNoZoom()) { /* used for drawing guides and decoration  */
    ...
    drawArc: function (center, radiusX, radiusY, startAngle, endAngle, anti) {
        if (this.webgram.rootContainer.isNoZoom()) {
            var startPoint = new Webgram.Geometry.Point(
                    center.x + radiusX * Math.cos(startAngle),
                    center.y + radiusY * Math.sin(startAngle));

            var endPoint = new Webgram.Geometry.Point(
                    center.x + radiusX * Math.cos(endAngle),
                    center.y + radiusY * Math.sin(endAngle));
            
            startPoint = this.transformDirect(startPoint);
            endPoint = this.transformDirect(endPoint);
    ...
    drawBezier: function (point1, point2, controlPoint1, controlPoint2) {
        if (this.webgram.rootContainer.isNoZoom()) { /* used for drawing guides */
            point1 = this.transformDirect(point1);
     ...
     * that contains all the shape points of this element.
     * This is a very important method of an element and therefore
     * it should be carefully implemented. <br><br>
     * <em>(must be overridden)</em>
     * @param {Boolean} transformed if set to <tt>true</tt>,
     * the transformed (relative to the parent) bounding rectangle will be returned,
     * otherwise the untransformed (relative to this element) bounding rectangle
     * will be returned
    ...
    /**
     * Relocates and resizes the element so that it fits the given rectangle.
     * The rectangle is relative to the parent. Implement this method if
     * you want this element to support grouping.<br><br>
     * <em>(should be overridden)</em> 
     * @param {Webgram.Geometry.Rectangle} rectangle the area which the
     * element has to fit
     */
    ...
    beginShapeChange: function (noUndoCheckPoint) {
        if (!this._shapeChanged) {
            ...
            if (!noUndoCheckPoint && this.webgram && this.webgram.userInteraction() && !this.isBeingCreated()) {
    ...
    /**
     * Enables or disables the interactive editing for this element.<br><br>
     * <em>(should be overridden)</em>
    ...
    /**
     * Returns the absolute (apparent) location of this element.
     * @returns {Webgram.Geometry.Point} the absolute location
     */
    getAbsLocation: function () {
        var origin = Webgram.Geometry.Point.zero();
        
        var drawingElement = this;
        while (drawingElement._parent) {
            origin = drawingElement.transformDirect(origin);
            
            drawingElement = drawingElement._parent;
        }
    ...
    _flipHorizontally: function () {
        /* add an undo check point */
        if (this.webgram && this.webgram.userInteraction() && !this.isBeingCreated()) {
            this.webgram.saveUndoCheckPoint(this, 'change');
        }
    ...
    /**
     * Computes the absolute (apparent) transform parameters.<br><br>
     * <em>(should not be overridden)</em>
     * @returns {Object} the absolute transform parameters:<ul>
     *  <li><tt>x</tt>: the x coordinate of the absolute location</li>
     *  <li><tt>y</tt>: the y coordinate of the absolute location</li>
     *  <li><tt>rotationAngle</tt>: the absolute rotation angle</li>
     *  <li><tt>flippedHorizontally</tt>: the absolute horizontal flip state</li>
     *  <li><tt>flippedVertically</tt>: the absolute vertical flip state</li>
     * </ul>
     */
    getAbsTransforms: function () {
        var origin = Webgram.Geometry.Point.zero();
        var rotationAngle = 0;
        ...
        var drawingElement = this;
        while (drawingElement._parent) { /* while drawingElement is not root container */
            origin = drawingElement.transformDirect(origin);
            rotationAngle = drawingElement._rotationAngle + rotationAngle;
            if (drawingElement.isFlippedHorizontally()) {
                flippedHorizontally = !flippedHorizontally;
            }
            if (drawingElement.isFlippedVertically()) {
        ...
        return {
            x: origin.x,
            y: origin.y,
            rotationAngle: Webgram.Geometry.normalizeAngle(rotationAngle),
            flippedHorizontally: flippedHorizontally,
            flippedVertically: flippedVertically
        };
    },
    ...
    snapToSiblings: function (point, snapVisualFeedbackList, hint) {
        ...
        var snappedXElement = null;
        var snappedYElement = null;
        ...
        if (snappedPointX != null) {
            if (snappedPointY != null) {
                ...
                if (this.webgram) {
                    this.webgram.onDrawingElementsInteract.trigger(this, snappedXElement, 'snap');
                    if (snappedXElement !== snappedYElement) {
                        this.webgram.onDrawingElementsInteract.trigger(this, snappedYElement, 'snap');
                    }
                }
     ...
     * <em>(should not be overridden)</em>
     * @param {Boolean} excludeDependent set to <tt>true</tt> to exclude dependent
     * elements from the returned list
    ...
    getSiblings: function (excludeDependent) {
        ...
        for (var i = 0; i < this._dependentDrawingElements.length; i++) {
            dependentElements[this._dependentDrawingElements[i].element.getId().toString()] = true;
        }
            
        for (i = 0; i < this._parent._drawingElements.length; i++) {
            var drawingElement = this._parent._drawingElements[i];
            if (excludeDependent && dependentElements[drawingElement.getId().toString()]) {
                continue;
            }
            
            siblings.push(drawingElement);
        }
    ...
    /**
     * Returns the elements that depend (directly or indirectly).<br><br>
     * <em>(should not be overridden)</em>
     * @returns {Array} the list of dependent elements
     */
    getDependentElements: function () {
        if (this._getDependentElementsLock) {
            return [];
        }
        ...
        while (true) {
            var tempDependentElements = [];
            
            for (var i = 0; i < elements.length; i++) {
                var element = elements[i];
                
                for (var j = 0; j < element._dependentDrawingElements.length; j++) {
                    var tempElement = element._dependentDrawingElements[j].element;
                    
                    /* avoid following recurrent dependencies */
                    if (Webgram.Utils.indexOf(dependentElements, tempElement) >= 0) {
                        continue;
                    }
                    
                    tempDependentElements.push(tempElement);
                }
            }
            
            if (tempDependentElements.length === 0) {
                break; /* no further parsing */
            }
            
            dependentElements = dependentElements.concat(tempDependentElements);
            elements = tempDependentElements;
        }
    ...
    updateDependentElements: function () {
        if (this._updateDependentElementsLock) {
            return [];
        }
        ...
        /* avoid recurrent dependency updating using a lock */
        this._updateDependentElementsLock = true;
        ...
        delete this._updateDependentElementsLock;
    ...
    finishDependentChangeEvents: function () {
        ...
        Webgram.Utils.forEach(this._dependentDrawingElements, function (dependent) {
            dependent.element.finishChangeEvents();
            dependent.element.finishDependentChangeEvents();
        });
    },
    ...
    // TODO jsdoc
    duplicate: function (add, overrideJson) {
        var json = this.toJson();
        Webgram.Utils.update(json, overrideJson);
        
        this.onDuplicate.trigger();

        var drawingElement = new this.constructor(); /* create an instance of this class with default properties */
        drawingElement._duplicating = true;
        
        drawingElement.fromJson(json);
        
        if (add) {
            this._parent._addDrawingElement(drawingElement, null, true);
        }
    ...
    /**
     * Prepares a json object with the class of this element.
     * If this method returns <tt>undefined</tt>, the corresponding part of the json object
     * will not exist.<br><br>
     * <em>(should not be overridden)</em>
     * @returns {Object} the json object with the class information
     */
    classToJson: function () {
    ...
    _fieldsToJson: function () {
        ...
        Webgram.Utils.forEach(this._jsonFields, function (jsonField) {
            var value = Webgram.Utils.getField(jsonField.field, undefined, this);
            
            value = Webgram.Utils.clone(value);
            Webgram.Utils.setField(jsonField.name, value, json);
        }, this);
    ...
    _fieldsFromJson: function (json) {
        Webgram.Utils.forEach(this._jsonFields, function (jsonField) {
            var value = Webgram.Utils.getField(jsonField.name, undefined, json);
            
            value = Webgram.Utils.clone(value);
            Webgram.Utils.setField(jsonField.field, value, this);
        }, this);
    },
     ...
     * and the <tt>name</tt> of the field as the second argument
    ...
    _funcsToJson: function () {
        ...
        Webgram.Utils.forEach(this._jsonFuncs, function (funcs) {
    ...
    toJson: function () {
        ...
        var classJson = this.classToJson();
    ...
    fromJson: function(json) {
        ...
        if (json.location !== undefined) {
            this.locationFromJson(json.location);
        }
        ...
        this.onRestore.trigger(json);
        // TODO remove the undo checkpoints related to this DE if not undo action
    },
     ...
     * @returns {Webgram.DrawingElementId} the given identifier of this element.
     */
    getId: function () {
        return this._id;
    },
     ...
     * @param {Webgram.DrawingElementId} id the new given identifier;
     * if passed as anything else than {@link Webgram.DrawingElementId},
    ...
    /**
     * Tells whether this drawing element is a special drawing element or not.
     * @returns {Boolean} <tt>true</tt> for a special element, <tt>false</tt> otherwise
     */
    isSpecial: function () {
        return this._id && this._id.isSpecial();
    },
     ...
     * @param {Number} focus one of the Webgram.DrawingElement.FOCUS_* values or a greater, user-defined value
     */
    setFocus: function (focus) {
        ...
        /* when selecting an element, also select its parent */
        if (focus === Webgram.DrawingElement.FOCUS_SELECTED && this._parent && this._parent._parent) {
            this._parent.setFocus(Webgram.DrawingElement.FOCUS_SELECTED);
        }
    },
    ...
    _setWebgram: function (webgram) {
        ...
        if (this._id == null) {
            var id = webgram.getNextIdSequence();
            var idStr = Array(4 - String(id).length + 1).join(0) + id;
            this.setId('#' + idStr);
        }
    },
...
/** @lends Webgram.DrawingElement */ {
    ...
    /** Represents the selected focus type of a drawing element when part of a multiple selection. */
    FOCUS_SELECTED_MULTIPLE: 3,
    /* any greater values are user specific */
    
    onExtend: function (childClass) {
        Webgram.DrawingElements.registerClass(childClass, childClass.getName());
    }
});


Webgram.DrawingElementId = Webgram.Class.extend( /** @lends Webgram.DrawingElementId.prototype */ {
    /**
     * A wrapper class that represents an id of a drawing element.
     * @constructs Webgram.DrawingElementId
     * @extends Webgram.Class
     * @param {primitive} id a given identifier value, can be <tt>null</tt>
     */
    initialize: function DrawingElementId(value) {
        this._value = value;
    },
    
    /**
     * Tells if the id is <em>special</em> or not.
     * Special ids are assigned to special drawing elements.
     * Special ids are string values and start with a <tt>@</tt> character.
     * @returns {Boolean} <tt>true</tt> if the id is special, <tt>false</tt> otherwise
     */
    isSpecial: function () {
        return this._value && this._value.toString().indexOf('@') === 0 || false;
    },

    /**
     * Tells if the id is <em>local</em> or not.
     * Local ids are ids assigned automatically
     * to drawing elements when added to a Webgram.
     * Local ids are string values and start with a <tt>#</tt> character.
     * @returns {Boolean} <tt>true</tt> if the id is local, <tt>false</tt> otherwise
     */
    isLocal: function () {
        return this._value && this._value.toString().indexOf('#') === 0 || false;
    },
     ...
     * @returns {primitive} the value of this id
     */
    valueOf: function () {
        return this._value;
    },
    ...
    /**
     * Compares two ids.
     * @param {Webgram.DrawingElementId} anotherId the other id
     * @returns {Boolean} <tt>true</tt> if the two ids
     * are equal, <tt>false</tt> otherwise
     */
    equals: function (anotherId) {
        if (anotherId == null || !anotherId.valueOf) {
            return false;
        }
        
        return this.valueOf() === anotherId.valueOf();
    }
});
