 ...
 */
Webgram.Class = function (constructor, parentClass, instanceMembers, classMembers) {
    if (!constructor) { /* constructor not given */
        constructor = instanceMembers && instanceMembers.initialize || null;
    }
    
    if (!constructor) { /* constructor not supplied at all, adding a default one */
    ...
    var className;
    try {
        className = constructor.toString().match(/^function\s*([^\s(]+)/)[1];
    }
    catch (e) {
        className = constructor.name || 'anonymous';
    }

    /* wrap the constructor so that
     * the super constructor is available */
    if (parentClass) {
        var oldConstructor = constructor;
        constructor = function () {
            var tmp = this.super;
            this.super = parentClass;
    ...
    var newPrototype;
    if (parentClass) {
        newPrototype = this._Inherit(parentClass.prototype);
        ...
        for (key in instanceMembers) {
            if (instanceMembers.hasOwnProperty(key)) {
                ...
                if (typeof instanceMember === 'function' && typeof parentClass.prototype[key] === "function") {
                    newPrototype[key] = function (func, key) {
                        return function () {
                            var tmp = this.super;
                            this.super = parentClass.prototype[key];
        ...
        if (instanceMembers) {
            newPrototype = this._Inherit(instanceMembers);
        }
        else {
    ...
    if (newPrototype !== constructor.prototype) {
        for (key in constructor.prototype) {
            if (constructor.prototype.hasOwnProperty(key) && !newPrototype.hasOwnProperty(key)) {
                var member = constructor.prototype[key];
                
                if (typeof member === 'function' && parentClass && typeof parentClass.prototype[key] === "function") {
                    newPrototype[key] = function (func, key) {
                        return function () {
                            var tmp = this.super;
                            this.super = parentClass.prototype[key];
    ...
    if (classMembers) {
        for (key in classMembers) {
            if (classMembers.hasOwnProperty(key)) {
                constructor[key] = classMembers[key];
            }
        }
    }
    ...
    if (!newPrototype.hasOwnProperty('toString')) {
 ...
 * a new class, but rather augments an existing one with additional members.
 * The <tt>instanceof</tt> operator will only work properly for the
 * classes inherited with the {@link Webgram.Class} function, defined above.
 * @param {Function} constructor the constructor function of the class to augment
 * @param {Function} parentClass the class to take the augmenting members from
 ...
 */
Webgram.Augment = function (constructor, parentClass, instanceMembers, classMembers) {
    var key = null;
    ...
    for (key in instanceMembers) {
        if (instanceMembers.hasOwnProperty(key) && !childClass.prototype.hasOwnProperty(key)) {
    ...
    for (key in parentClass) {
        if (parentClass.hasOwnProperty(key) && !childClass.prototype.hasOwnProperty(key)) {
    ...
    for (key in classMembers) {
        if (classMembers.hasOwnProperty(key) && !childClass.prototype.hasOwnProperty(key)) {
