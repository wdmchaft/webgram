...
Webgram = function (htmlElement, canvas, settings) {
     ...
     * Handlers receive the following arguments: <tt>(drawingElement, operation, byKeyboard)</tt>,
     * where <tt>byKeyboard</tt> is set to <tt>true</tt> if the action was triggered by a keyboard combination.
     * @see Webgram#doUndo
     * @type Webgram.Event
     */
    this.onUndoAction = new Webgram.Event('undo action', this); /* (drawingElement, operation, byKeyboard) */
     ...
     * @type Webgram.Event
     */
    this.onRedoAction = new Webgram.Event('redo action', this); /* (drawingElement, operation, byKeyboard) */
    ...
    this._undoCheckPointsEnabled = true;
...
Webgram.prototype = {
     ...
     * @returns {Boolean} <tt>true</tt> if the action succeeded, <tt>false</tt> otherwise
     */
    doUndoAction: function (byKeyboard) {
        if (this.getSetting('maxUndoLevels') > 0) { /* undo enabled */
            if (this._undoPosition > 0) {
                ...
                var oldUndoCheckPointsEnabled = this._undoCheckPointsEnabled;
                this._undoCheckPointsEnabled = false;
                
                var checkPoint = this._undoCheckPoints[--this._undoPosition];
                
                switch (checkPoint.operation) {
                    case 'add':
                        checkPoint.drawingElement._parent.removeDrawingElement(checkPoint.drawingElement);
                        
                        break;
                       
                    case 'remove':
                        checkPoint.drawingElement.fromJson(checkPoint.json);
                ...
                this._undoCheckPointsEnabled = oldUndoCheckPointsEnabled;
                
                this.onUndoAction.trigger(checkPoint.drawingElement, checkPoint.operation, byKeyboard === true);
     ...
     * @returns {Boolean} <tt>true</tt> if the action succeeded, <tt>false</tt> otherwise
     */
    doRedoAction: function (byKeyboard) {
        // TODO implement me
    },
    ...
    /**
     * Adds an <em>undo check point</em> by saving the state of a drawing element,
     * just before an operation is applied to it.
     * @param {Webgram.DrawingElement} drawingElement the element that will be modified
     * @param {String} operation the operation that will be applied to the element
     * (<tt>add</tt>, <tt>remove</tt> or <tt>change</tt>)
     */
    saveUndoCheckPoint: function (drawingElement, operation) {
        if (!this._undoCheckPointsEnabled) {
            return;
        }
        
        /* we lose all the check points that are ahead of the current position */
        if (this._undoPosition < this._undoCheckPoints.length - 1) {
            this._undoCheckPoints.splice(this._undoPosition + 1, this._undoCheckPoints.length);
        }
        
        /* don't allow more than maxUndoLevels check points */
        if (this._undoCheckPoints.length >= this.getSetting('maxUndoLevels')) {
     ...
     * @param {Boolean} enabled <tt>true</tt> to enable undo checkpoints, <tt>false</tt> to disable them
     */
    setUndoCheckPointsEnabled: function (enabled) {
        this._undoCheckPointsEnabled = enabled;
    },
