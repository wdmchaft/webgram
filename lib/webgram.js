...
Webgram.prototype = {
    ...
    saveUndoCheckPoint: function (drawingElements, operation) {
        ...
        var allDrawingElements = [];
        for (var i = 0; i < drawingElements.length; i++) {
            var drawingElement = drawingElements[i];
            if (Webgram.Utils.indexOf(allDrawingElements, drawingElement) < 0) {
                allDrawingElements.push(drawingElement);
            }

            var dependentElements = drawingElement.getDependentElements();
            for (var j = 0; j < dependentElements.length; j++) {
                var dependentElement = dependentElements[j];
                if (Webgram.Utils.indexOf(allDrawingElements, dependentElement) >= 0) {
                    continue;
                }
                
                allDrawingElements.push(dependentElement);
            }
        }
        
        /* parse all the gathered drawing elements and create
         * undo check point details for each of them */
        for (var i = 0; i < allDrawingElements.length; i++) {
            ...
            var index = parent ? parent.getDrawingElementIndex(drawingElement) : -1;
