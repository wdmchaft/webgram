...
Webgram = function (htmlElement, canvas, settings) {
     ...
     * The following types of interaction are currently defined:<ul>
     * <li><tt>snap</tt></li>
     * <li><tt>connect</tt></li>
     * <li><tt>disconnect</tt></li>
     * </ul><br>
     * Handlers receive the following arguments: <tt>(drawingElement1, drawingElement2, type)</tt>
     * @type Webgram.Event
     */
    this.onDrawingElementsInteract = new Webgram.Event('drawing elements interact', this); /* (drawingElement1, drawingElement2, type) */
     ...
     * Handlers receive the following arguments: <tt>(drawingElements, direction, fine)</tt>.
     * @see Webgram#doMove
     * @type Webgram.Event
     */
    this.onMoveAction = new Webgram.Event('move action', this); /* (drawingElements, direction, fine) */
...
Webgram.prototype = {
    ...
    /**
     * Recursively searches for the drawing element with the given id.
     * <em>(should not be overridden)</em>
     * @param {Webgram.DrawingElementId} id the id to search for;
     * if passed as anything else than {Webgram.DrawingElementId},
     * will be wrapped in one
    ...
    getDrawingElementById: function (id) {
        if (!(id instanceof Webgram.DrawingElementId)) {
            id = new Webgram.DrawingElementId(id);
        }
        
        var deId = this.rootContainer.getId();
        if (deId && deId.equals(id)) {
            return this.rootContainer;
        }
        
        return this.rootContainer.getDrawingElementById(id);
    },
    ...
    getDrawingElements: function () {
        ...
        Webgram.Utils.forEach(this.rootContainer.getDrawingElements(), function (drawingElement) {
            if (drawingElement.isSpecial()) {
                return;
            }
    ...
    doUndoAction: function () {
        if (this.getSetting('maxUndoLevels') > 0) { /* undo enabled */
            if (this._undoPosition > 0) {
                ...
                var drawingElements = [];
                for (var i = checkPoint.length - 1; i >= 0; i--) {
                    var info = checkPoint[i];
                    
                    switch (info.operation) {
                        case 'add': {
                            var drawingElement = this.getDrawingElementById(info.id);
                            if (!drawingElement) {
                                continue;
                            }
                            ...
                            parent.remDrawingElement(drawingElement);
                        ...
                        case 'remove': {
                            var elementClass = Webgram.DrawingElements.findClass(info.json.cls);
                            if (!elementClass) {
                                continue;
                            }
                            
                            var parent = this.getDrawingElementById(info.parentId);
                            if (!parent) {
                                continue;
                            }
                            
                            var drawingElement = new elementClass(elementClass);
                            drawingElement.fromJson(info.json);
                            
                            parent.addDrawingElement(drawingElement, info.index);
                            
                            /* replace the old id with the new one */
                            Webgram.Utils.visit(this._undoCheckPoints, function (path, parent, type, value) {
                                if (value instanceof Webgram.DrawingElementId && value.equals(info.id)) {
                                    parent[path[path.length - 1]] = drawingElement.getId(); 
                                }
                            });
                        ...
                        case 'index-change': {
                            ...
                            var index = parent.getDrawingElementIndex(drawingElement);
                            
                            parent.setDrawingElementIndex(drawingElement, info.index);
                ...
                if (checkPoint.length > 1) {
                    ...
                    operatoion = checkPoint[0].operation;
                }
                
                this.onUndoAction.trigger(drawingElements, operation);
    ...
    doRedoAction: function () {
        if (this.getSetting('maxUndoLevels') > 0) { /* undo enabled */
            if (this._undoPosition < this._undoCheckPoints.length) {
                ...
                this.onRedoAction.trigger(drawingElements, operation);
     ...
     * drawing elements just before an operation is applied to them.
     * @param {Array} drawingElement the element that will be modified
     ...
     */
    saveUndoCheckPoint: function (drawingElement, operation) {
        ...
        /* gather the dependent drawing elements */
        var drawingElements = drawingElement.getDependentElements();
        drawingElements.unshift(drawingElement);
        
        /* parse all the gathered drawing elements and create
         * undo check point details for each of them */
        var infoList = [];
        for (var i = 0; i < drawingElements.length; i++) {
            var de = drawingElements[i];
            ...
            var index = parent ? parent.getDrawingElementIndex(de) : -1;
            
            infoList.push({
                'operation': i == 0 ? operation : 'change',
                'json': json,
                'id': id,
        ...
        if (this._undoGroup > 0 && this._undoCheckPoints.length) {
            var lastCheckPoint = this._undoCheckPoints[this._undoCheckPoints.length - 1];
            
            Webgram.Utils.forEach(infoList, function (info) {
                lastCheckPoint.push(info);
            });
        }
        else {
            this._undoCheckPoints.push(infoList);
    ...
    /**
     * Use this method to begin an undo group
     * (i.e. more than one operations in a single undo check point). 
     * @see {Webgram#endUndoGroup}
     */
    beginUndoGroup: function () {
        if (this.getSetting('maxUndoLevels', 0) <= 0) { /* undo disabled */
            return;
        }
        
        /* don't begin an undo group if the mechanism is temporarily disabled */
        if (this._undoCheckPointsDisabled) {
            return;
        }
        ...
        /* create a new group if the last one is not empty */
        if (this._undoCheckPoints.length && this._undoCheckPoints[this._undoCheckPoints.length - 1].length > 0) {
            /* don't allow more than maxUndoLevels check points */
            while (this._undoCheckPoints.length >= this.getSetting('maxUndoLevels')) {
                this._undoCheckPoints.shift();
                this._undoPosition--;
    ...
    /**
     * Use this method to end an undo group
     * (i.e. more than one operations in a single undo check point). 
     * @see {Webgram#beginUndoGroup}
     */
    endUndoGroup: function () {
        ...
        /* remove the last undo group if empty */
        if (this._undoCheckPoints.length &&
            this._undoCheckPoints[this._undoCheckPoints.length - 1].length === 0) {
            
            this._undoCheckPoints.pop();
        }
    ...
    /**
     * TODO do we still use this method? we should use weak refs in undo list
    ...
    /**
     * Registers the various handler functions for the JavaScript keyboard and mouse events.
     ...
     * <em>(should not be overridden)</em>
     */
    enableKeyMouseHandlers: function () {
        var webgram = this;
     ...
     * @type {String} if given, will test only for a specific type
     * of user interaction; possible values are <tt>keyboard</tt>,
     * <tt>mouse</tt>, <tt>focus</tt> and <tt>action</tt>.
     * @returns {Boolean}
     */
    userInteraction: function (type) {
        if (type != null) {
            switch (type) {
                case 'keyboard':
    ...
    /**
     * Creates a mini Webgram for this webgram.<br><br>
     * <em>(should not be overridden)</em>
     * @param {HtmlElement} htmlElement the <em>HTML</em> element to be used
     * to receive the mouse and keyboard events for the mini webgram;
     * this is normally the <em>HTML Canvas</em> element whose 2D context
     * is used to create the canvas object passed as the second argument
     * @param {Webgram.Canvas} canvas the canvas to be used paint the mini Webgram
     */
    setMiniWebgram: function (htmlElement, canvas) {
        canvas._fonts = this.getSetting('fonts');
        
        this._miniWebgram = new Webgram.MiniWebgram(htmlElement, canvas);
        this._miniWebgram.mainWebgram = this;
        this._updateMini();
        
        if (this.htmlElement.tabIndex === 0) {
            /* if handlers attached to the main webgram,
             * also attach them to the mini webgram */
            
            this._miniWebgram.enableKeyMouseHandlers();
        }
    },

    _getMiniParams: function () {
        var marginFactor = this._miniWebgram.marginFactor;
        var thisMargins = this.rootContainer.getBoundingRectangle();
    ...
    _drawMini: function () {
        ...
        this.rootContainer.draw();
        this.rootContainer._mini = false;
    ...
    _updateFromMini: function () {
        ...
        var visibleBounds = this._miniWebgram.visibleArea.getBoundingRectangle();
        var topLeft = visibleBounds.getTopLeft();
        var bottomRight = visibleBounds.getBottomRight();
        topLeft = this._miniWebgram.visibleArea.transformDirect(topLeft);
        bottomRight = this._miniWebgram.visibleArea.transformDirect(bottomRight);
    ...
    /**
     * Sets the tooltip of the HTML element of this Webgram.
     * @param {String} text the tooltip text to assign
     */
    ...
    /**
     * Clearsthe tooltip of the HTML element of this Webgram.
     */
...
Webgram.Class = (function () {
    ...
    if (Object.create) {
        inherit = function (parentClass) {
            return Object.create(parentClass.prototype);
        };
    }
    else {
        inherit = function (parentClass) {
            eval('var Inherited = function ' + parentClass.getName() + '() {};'); /* this will give the function a name */
            Inherited.prototype = parentClass.prototype;
            Inherited.prototype.constructor = parentClass;
    ...
    /* provide a function to retrieve the class name */
     ...
     * special instance members:<ul>
     *  <li><tt>toString</tt>: a string conversion method</li>
     * </ul>
     * @param {Object} classMembers a dictionary with class members for the new class;
     * special class members:<ul>
     *  <li><tt>name</tt>: an explicit name of the class</li>
     *  <li><tt>onExtend</tt>: a function that is called whenever the
     *  class is extended (directly or indirectly); receives the child class as argument</li>
     * </ul>
    ...
    Class.extend = function (instanceMembers, classMembers) {
        ...
        if (constr == null) { /* constructor not supplied, wrap parent's constructor */
            /* use eval to assign a name to the wrapped constructor */
            eval('constr = function Inherited' + this.getName() + ' () {parentConstr.apply(this, arguments);}');
        }
        
        /* determine the class name */
        var className = classMembers && classMembers.name || constr.name;
        if (!className) {
            className = constr.toString().match(/^function\s*([^\s(]+)/);
        ...
        var newPrototype = inherit(parentConstr);

        /* add the supplied instance members to the new prototype */
        var i, key, keys = Webgram.Utils.keys(instanceMembers);
        ...
        keys = Webgram.Utils.keys(classMembers);
        for (i = 0; i < keys.length; i++) {
            var key = keys[i];
        ...
        constr.parentClass = parentConstr;
        constr.parent = parentConstr.prototype;
        ...
        if (newPrototype.toString === {}.toString) {
        ...
        constr.getName = Class.getName;
        constr.__name__ = className;
        
        /* call the onExtend function, if supplied */
        while (parentConstr) {
            if (parentConstr.onExtend) {
                parentConstr.onExtend(constr);
            }
            
            parentConstr = parentConstr.parentClass;
        }
    ...
    /**
     * This creates a new class by augmenting an existing one
     * with the members of a given class, rather than extending one.
     * Augmentation classes may provide an <tt>initialize</tt> method,
     * but it will be called with no arguments.
     * The <tt>instanceof</tt> operator will not work properly with augmented classes.
     * @memberof Webgram.Class
     * @param {Function} augClass an optional class to take the augmenting members from
     * @param {Object} instanceMembers an optional dictionary with instance members to add to the class
     * @param {Object} classMembers an optional dictionary with class members to add to the class
     * @returns {Function} the new class
     */
    Class.augment = function (augClass, instanceMembers, classMembers) {
        var constr = this.extend(instanceMembers, classMembers);

        var key = null;
        if (augClass) {
            /* add the instance members of the parent to the child */
            for (key in augClass.prototype) {
                if (key === 'constructor' || key === 'toString') {
                    continue;
                }
                
                constr.prototype[key] = augClass.prototype[key];
            }
        }
        
        if (augClass || (instanceMembers && instanceMembers.initialize)) {
            /* wrap the constructor so that the additional
             * augmentation constructor gets called as well */
            
            /* use eval to assign a name to the wrapped constructor */
            var className = constr.getName();
            var oldConstr = constr;
            var funcCode = 'constr = function ' + className + ' () {';
            funcCode += 'oldConstr.apply(this, arguments);';
            if (augClass) {
                funcCode += 'augClass.call(this);';
            }
            funcCode += '};';
            
            eval(funcCode);
            
            /* transmit various attributes */
            constr.prototype = oldConstr.prototype;
            constr.prototype.constructor = constr;
            constr.parent = oldConstr.parent;
            constr.parentClass = oldConstr.parentClass;

            /* add a toString method to the new constructor */
            constr.toString = function () {
                return className;
            };
    ...
    /* make the various OOPmethods available on the Webgram class */
    Webgram.extend = Class.extend;
    Webgram.augment = Class.augment;
    Webgram.getName = Class.getName;
    Webgram.__name__ = 'Webgram';
...
Webgram.Utils = {
    ...
    clone: function (original) {
        if (original === undefined ||
            ...
            var i, key, keys = Webgram.Utils.keys(original);
            for (i = 0; i < keys.length; i++) {
                key = keys[i];
    ...
    update: function (dest, source) {
        function updateRec(destParent, source, fieldName) {
            if (source instanceof Array) {
                ...
                var i, key, keys = Webgram.Utils.keys(source);
    ...
    visit: function (value, callback) {
        function visitRec(value, fieldPath, parent) {
            ...
            if (value === undefined) {
                ...
                if (result === undefined) {
                    var i, key, keys = Webgram.Utils.keys(value);
                    for (i = 0; i < keys.length; i++) {
    ...
    /**
     * Calls a function on each of the elements of an array.
     * @param {Array} array the array to parse
     * @param {Function} func the function to call on each element;
     * it will be called with the element as the first argument,
     * the index as the second argument and the array as the third argument
     * @param {Object} thisArg if given, will be used as "this"
    ...
    /**
     * Loads a list of scripts, calling a supplied function
    ...
    loadScripts: function (srcList, onLoad, sync, reload) {
        ...
        else {// TODO document.write() is fucked up
            var html = '';
    ...
    // TODO jsdoc
    temporarySet: function (obj, getter, setter, value) {
        obj.__temp = obj.__temp || [];
        var curVal = getter.call(obj);
        if (curVal !== value) {
            obj.__temp.push({
                origVal: curVal,
                setter: setter
            ...
            obj.__temp.push(null);
        }
    },
    
    // TODO jsdoc
    temporarySetAttr: function (obj, attr, value) {
        obj.__temp = obj.__temp || [];
        var curVal = obj[attr];
        if (curVal !== value) {
            obj.__temp.push({
    ...
    // TODO jsdoc
    temporaryRestore: function (obj) {
        if (obj.__temp && obj.__temp.length) {
            var temp = obj.__temp.pop();
            if (temp) {
                if (temp.setter) {
                    temp.setter.call(obj, temp.origVal);
                }
                else { /* temp.attr */
                    if (temp.origVal === undefined) {
                        delete obj[temp.attr];
                    }
                    else {
                        obj[temp.attr] = temp.origVal;
                    }
                }
            }
        }
...
Webgram._scriptSrcList = (function () {
    var srcList = [
        ...
        'control-points/action-menu-item.js',
        ...
        'drawing-elements/group-element.js',
