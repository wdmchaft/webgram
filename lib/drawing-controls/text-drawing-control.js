...
Webgram.DrawingControls.TextDrawingControl =
        Webgram.DrawingControl.extend( /** @lends Webgram.DrawingControls.TextDrawingControl.prototype */ {
            
    /**
     * A drawing control that enables interactive text editing on the canvas.
     * @constructs Webgram.DrawingControls.TextDrawingControl
     * @extends Webgram.DrawingControl
     */
    initialize: function TextDrawingControl(rootContainer) {
        Webgram.DrawingControls.TextDrawingControl.parentClass.call(this, rootContainer);
    ...
    /**
     * Sets up the environment for editing text on a drawing element.
     ...
     * @param {Webgram.Geometry.Rectangle} box the text editing area, relative to the drawing element
     * @param {Webgram.Styles.TextStyle} textStyle the text style to use
     * @param {Number} angle the angle at which the text editing will be done, relative to the drawing element
     */
    configure: function (drawingElement, textField, box, textStyle, angle) {
        ...
        /* box */
        this._box = box || drawingElement.getBoundingRectangle();

        /* angle */
        this._angle = drawingElement.getRotationAngle();
        if (angle != null) {
            if (drawingElement.isFlippedHorizontally() !== drawingElement.isFlippedVertically()) {
    ...
    handleMouseDown: function (point, button, modifiers) {
        ...
        if (this._textInputArea.transformedPointInside(point)) {
    ...
    _drawElements: function () {
        ...
        this.rootContainer.drawRect(this.getWebgram().getVisibleArea().getShrunk(-1));
...
Webgram.DrawingControls.TextDrawingControl._TextInputArea =
        Webgram.DrawingElements.RectangularElement.extend({
            
    /*
     * Represents the area used to edit the text.
     */
    initialize: function _TextInputArea(textDrawingControl) {
        ...
        Webgram.DrawingElements.RectangularElement.call(this, width, height);
        
        this.setId('@text-input-area');
        
        this._setLocation(boxCenter, false);
        ...
        this._setRotationAngle(textDrawingControl._angle, false);
        if (de.isFlippedHorizontally()) {
            this.flipHorizontally();
        }
        if (de.isFlippedVertically()) {
            this.flipVertically();
        }
    ...
    pointInside: function (point) {
        ...
        var rect = this.getBoundingRectangle().getShrunk(-this.margin);
