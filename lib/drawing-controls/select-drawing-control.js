...
Webgram.DrawingControls.SelectDrawingControl = 
    ...
    _finishMultipleSelection: function (mouseDownPoint) {
        ...
        var drawingElements = this._msContainerElement._drawingElements.slice();
        for (var i = 0; i < drawingElements.length; i++) {
            var drawingElement = drawingElements[i];
        ...
        if (drawingElements.length + this._msContainerElement._drawingElements.length > 1) {
            ...
            for (i = 0; i < drawingElements.length; i++) {
                this._msContainerElement._drawingElementIndexes.push(this.rootContainer.getDrawingElementIndex(drawingElements[i]));
            }
            
            /* place the MSCE right before the first selected element */
            this.rootContainer._setDrawingElementIndex(this._msContainerElement, this._msContainerElement._drawingElementIndexes[0]);
        ...
        else if (drawingElements.length === 1) {
    ...
    _cancelMultipleSelection: function () {
        ...
        for (var i = 0; i < drawingElements.length; i++) {
            if (drawingElements.length === 1) {
                this.selectedDrawingElement = drawingElements[i];
                drawingElements[i].setFocus(Webgram.DrawingElement.FOCUS_SELECTED);
            }
            else {
                drawingElements[i].setFocus(Webgram.DrawingElement.FOCUS_NONE);
            }
            
            this.rootContainer._addDrawingElement(drawingElements[i]);
        }
        ...
        for (i = 0; i < drawingElements.length; i++) {
            var drawingElement = drawingElements[i];
            drawingElement._parent._setDrawingElementIndex(drawingElement, indexes[i]);// TODO this might trigger index events and it should not!
        }
    }
},
