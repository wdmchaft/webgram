...
Webgram.DrawingControls.SelectDrawingControl = 
    ...
    _markMultipleSelected: function (drawingElements) {
        ...
        if (drawingElements) {
            ...
            msBoundingRectangle = this._msContainerElement.translateDirect(this._msContainerElement.getBoundingRectangle());
            ...
            for (i = 0; i < drawingElements.length; i++) {
                ...
                if ((msBoundingRectangle.x1 <= boundingRectangle.x1) &&
                    ...
                    // TODO this is triggered also when the child wasn't previously selected
    ...
    _remFromMultipleSelection: function (temporary) {
         ...
         * temporary indicates if this is a
         * temporary multiple-selection removal, in which case
         * the multiple-selection element is not removed */
        ...
        /* create a copy of the list of children in the multiple selection element */
        for (i = 0; i < this._msContainerElement._drawingElements.length; i++) {
            pendingToRem.push(this._msContainerElement._drawingElements[i]);
        }
        
        for (i = 0; i < pendingToRem.length; i++) {
            this.rootContainer._addDrawingElement(pendingToRem[i]);
            
            if (temporary) {
                pendingToRem[i].setFocus(Webgram.DrawingElement.FOCUS_SELECTED_MULTIPLE);
            }
        }
        ...
        if (indexes != null) {
            ...
            for (i = 0; i < pendingToRem.length; i++) {
                var drawingElement = pendingToRem[i];
                drawingElement._parent._setDrawingElementIndex(drawingElement, index + i);// TODO this might trigger index events and it should not!
            }
        }
...
Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer =
    ...
    handleMouseDown: function (point, button, modifiers) {
        ...
        for (var i = this._drawingElements.length - 1; i >= 0; i--) {
            ...
            if (drawingElement.transformedPointInside(transformedPoint)) {
                hoveredDrawingElement = drawingElement;
                break;
            }
        }
    ...
    isSnapExternallyEnabled: function () {
        return this._drawingElements.length > 0;
    },
