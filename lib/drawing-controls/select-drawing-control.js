...
Webgram.DrawingControls.SelectDrawingControl = 
    ...
    handleMouseMove: function (point, modifiers) {
        ...
        if (hoveredActionMenuItem !== this.hoveredActionMenuItem) { /* not the same hovered action menu item */
            /* remove hover from the currently hovered element */
            if (this.hoveredActionMenuItem && this.hoveredActionMenuItem.getFocus() === Webgram.ActionMenuItem.FOCUS_HOVERED) {
                this.hoveredActionMenuItem.setFocus(Webgram.ActionMenuItem.FOCUS_NONE);
                this.hoveredActionMenuItem.onRemove.unbind(Webgram.DrawingControls.SelectDrawingControl._whenPointRemoved);
            }
            
            /* hover the new AMI, unless it's selected */
            if (hoveredActionMenuItem && hoveredActionMenuItem.getFocus() === Webgram.ActionMenuItem.FOCUS_NONE) {
                hoveredActionMenuItem.setFocus(Webgram.ActionMenuItem.FOCUS_HOVERED);
...
Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer =
        Webgram.DrawingElements.ContainerElement.extend({

    initialize: function _MultipleSelectionContainer(x, y, width, height) {
        Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.parentClass.call(this,
                ':special:multiple-selection-container', x, y, width, height);
