...
Webgram.DrawingControls.SelectDrawingControl = 
    ...
    getSelectedDrawingElements: function () {
        if (this._msContainerElement) {
            return this._msContainerElement._drawingElements.slice();
        }
        ...
        if (drawingElements && drawingElements.length) {
            if (drawingElements.length === 1) { /* single selected element */
                ...
                if (this.getSetting('multipleSelectionEnabled')) {
                    ...
                    this._msContainerElement = new Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer();
    ...
    doPasteAction: function () {
        ...
        if (clipboard.type === 'json/drawing-elements') {
            ...
            if (selectedDrawingElements.length > 0) {
                location = selectedDrawingElements[0].getLocation();
            }
            
            var delta = this.getSetting('mainGrid.sizeX', 25);
            var deltaX = delta, deltaY = delta;
            if (location != null) {
                var oldLocation = drawingElements[0].getLocation();
                deltaX += location.x - oldLocation.x;
                deltaY += location.y - oldLocation.y;
            }
            
            for (i = 0; i < drawingElements.length; i++) {
                drawingElement = drawingElements[i];
                drawingElement.setLocation(drawingElement.getLocation().getTranslated(deltaX, deltaY));
            }
    ...
    handleMouseDown: function (point, button, modifiers) {
        ...
        if (button === 1) {
            ...
            if (this.selectedControlPoint) { /* clicked on a CP */
                ...
                if (modifiers.shift && (!this._msContainerElement || (this._msContainerElement.getRotationAngle() === 0))) { /* add to current selection */
                    if (this.selectedDrawingElement !== this._msContainerElement) {
                        if (this._msContainerElement) { /* existing multiple selection */
                            //this._cancelMultipleSelection(true);
                            ...
                            if (this.getSetting('multipleSelectionEnabled')) {
                                if (prevSelectedDrawingElement && prevSelectedDrawingElement !== this.selectedDrawingElement) {
                                    ...
                                    this.selectedDrawingElement.setFocus(Webgram.DrawingElement.FOCUS_SELECTED_MULTIPLE);
                                    this._msContainerElement = new Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer(point.x, point.y);
                                    this.rootContainer._addDrawingElement(this._msContainerElement);
                                    this._finishMultipleSelection(point);
                                }
                                else {
    ...
    handleMouseUp: function (point, button, modifiers) {
        ...
        if (button === 1) {
            ...
            if (this._msContainerElement && this._msContainerElement._drawingElements.length === 0) {
                this._finishMultipleSelection(point);
                this.invalidate();
    ...
    handleMouseMove: function (point, modifiers) {
        ...
        if (this._panMouseDownPoint) { /* used for panning */
            ...
            else if (this._msContainerElement && this._msContainerElement.isEditEnabled()) {
                /* continue an already started multiple selection */
                
                var relativePoint = this._msContainerElement.transformInverse(point);
                if (this._mouseDownPoint.x <= point.x) {
                    if (this._mouseDownPoint.y <= point.y) {
                        this._msContainerElement._setBottomRight(relativePoint, true);
                    }
                    else {
                        this._msContainerElement._setTopRight(relativePoint, true);
                    }
                }
                else {
                    if (this._mouseDownPoint.y <= point.y) {
                        this._msContainerElement._setBottomLeft(relativePoint, true);
                    }
                    else {
                        this._msContainerElement._setTopLeft(relativePoint, true);
                    }
                }
    ...
    _markMultipleSelected: function (drawingElements) {
        ...
        msBoundingRectangle = this._msContainerElement.getBoundingRectangle(true);
    ...
    _finishMultipleSelection: function (mouseDownPoint) {
        ...
        var drawingElements = this._msContainerElement.getDrawingElements();
        for (var i = 0; i < drawingElements.length; i++) {
            var drawingElement = drawingElements[i];
        ...
        for (i = 0; i < allDrawingElements.length; i++) {
            ...
            if (firstIndex == null) {
                 ...
                 * we'll use it for placing the multiple selection container */
                firstIndex = i;
            }
            drawingElements.push(drawingElement);
        }
        
        if (drawingElements.length + this._msContainerElement._drawingElements.length > 1) {
             /* more than one selected element, keep the msContainerElement */
            ...
            if (firstIndex > 0) {
                ...
                if (drawingElements[0]._prevSibling === this._msContainerElement) {
                    /* if the first previous sibling is the multiple selection container,
                     * use the last element in the selection instead */
                    
                    drawingElements[0]._prevSibling = 
                            this._msContainerElement._drawingElements[this._msContainerElement._drawingElements.length - 1];
                }
            }
            
            this._msContainerElement.setFocus(Webgram.DrawingElement.FOCUS_SELECTED);
            this.selectedDrawingElement = this._msContainerElement;
            ...
            /* add the new elements to the multiple selection container */
            for (i = 0; i < drawingElements.length; i++) {
                this._msContainerElement._addDrawingElement(drawingElements[i]);
            }
            
            /* place the multiple selection container in the position
             * of the first element of the group */
            this.rootContainer._setDrawingElementIndex(this._msContainerElement, firstIndex);
            
            this.onSelectionChange.trigger(this._msContainerElement.getDrawingElements());
        }
        else if (drawingElements.length === 1 /* && this._msContainerElement._drawingElements.length === 0 */) {
            /* a single selected element, select it directly instead of adding to multiple selection,
             * and destroy the existing multiple selection container */
    ...
    _cancelMultipleSelection: function () {
        ...
        for (var i = 0; i < drawingElements.length; i++) {
            ...
            if (drawingElements.length === 1) {
                this.selectedDrawingElement = drawingElement;
        ...
        /* remove the multiple selection container first */
        this.rootContainer._remDrawingElement(this._msContainerElement);
        this._msContainerElement = null;
    }
},
...
Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer =
        Webgram.DrawingElements.GroupElement.extend({

    initialize: function _MultipleSelectionContainer(x, y) {
        Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.parentClass.call(this, ':special:multiple-selection-container');
        ...
        this.onIndexChange.bind(function () {
             /* when changing the index of the multiple selection container,
    ...
    drawSelf: function () {
        /* the drawing is done via drawNoZoom */
    },
    ...
    setFocus: function (focus) {
        ...
        Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.parent.setFocus.call(this, focus);
    },
    ...
    isSnapExternallyEnabled: function () {
        return (Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.parent.isSnapExternallyEnabled.call(this) &&
    ...
    _overrideChildMembers: function (drawingElement) {
        drawingElement.getParent = Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.msGetParent;
        drawingElement.isEnclosed = Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.msIsEnclosed;
        drawingElement.locationToJson = Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.msLocationToJson;
        drawingElement.rotationAngleToJson = Webgram.DrawingControls.SelectDrawingControl._MultipleSelectionContainer.msRotationAngleToJson;
    },
    
    _restoreChildMembers: function (drawingElement) {
        delete drawingElement.getParent;
        delete drawingElement.isEnclosed;
...
/* static members */ {
    msGetParent: function () {
        /* temporary replacement for elements that are part of a multiple selection */
        
        return this._parent._parent;
    },

    msIsEnclosed: function () {
        /* temporary replacement for elements that are part of a multiple selection */
    ...
    // TODO msLocationFromJson:
    
    msRotationAngleToJson: function () {
        /* temporary replacement for elements that are part of a multiple selection */
        
        return this._parent.getRotationAngle() + this.getRotationAngle();
    },
    
    // TODO msRotationAngleFromJson
    
    // TODO msFlipToJson    
    // TODO msFlipFromJson
    // TODO getDrawingElementIndex
});
